...
NINJA_VERBOSE=OFF
NINJA_DRY_RUN=OFF
# FIXME: Do NOT have target install/strip, so set target install and manually strip to workaround.
...
if [ -z $OHOS_NDK_ROOT ];then
    echo "can not find OHOS_NDK_ROOT env, pls export you NDK root dir to OHOS_NDK_ROOT"
    exit -1
fi

function cmake_build() {
    if [ $1 = "armeabi-v7a with NEON" ] ;then
        BUILD_DIR=$SRC_DIR/build_dir/ohos/armeabi-v7a/$BUILD_TYPE/build
    else
        BUILD_DIR=$SRC_DIR/build_dir/ohos/$1/$BUILD_TYPE/build
    fi
    ...
    bash -c "cmake -G Ninja \
        -DCMAKE_TOOLCHAIN_FILE="$OHOS_NDK_ROOT/build/cmake/ohos.toolchain.cmake" \
        -DOHOS_STL=c++_static \
        -DOHOS_ARCH=$ARCH \
        -DOHOS_PLATFORM=OHOS \
    ...
    bash -c "${NINJA_CMD}"
    find $INSTALL_DIR -type d -name "bin" -exec find {} -type f \; | xargs $OHOS_NDK_ROOT/llvm/bin/llvm-strip
    find $INSTALL_DIR -type f -name "*.so" | xargs $OHOS_NDK_ROOT/llvm/bin/llvm-strip
}
