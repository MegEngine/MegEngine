...
PyObject* BFloat16Type::PyObj::py_new(
    ...
    if (!PyFloat_Check(x)) {
        PyErr_SetString(PyExc_TypeError, "BFloat16Type must be initialized wit float");
        return NULL;
    }
...
PyObject* BFloat16Type::PyObj::py_richcompare(PyObject* a, PyObject* b, int op) {
...
struct BFloat16Type::NpyType {
    ...
    static PyObject* getitem(void* data, void* ap) {
        return BFloat16Type::PyObj::from_bfloat16(*static_cast<BFloat16Type*>(data));
    }
...
int BFloat16Type::NpyType::setitem(PyObject* op, void* ov, void* ap) {
    if (PyLong_Check(op)) {
        ...
        static_cast<BFloat16Type*>(ov)->value = a;
    } else if (PyObject_IsInstance(op, (PyObject*)(&(BFloat16Type::PyObj::py_type)))) {
        static_cast<BFloat16Type*>(ov)->value = ((PyObj*)op)->obj.value;
    } else {
        PyErr_SetString(PyExc_ValueError, "input type must be int/float/bfloat16");
        return -1;
    }
    return 0;
}
...
bool BFloat16Type::NpyType::init() {
    descr = {
            PyObject_HEAD_INIT(0) & BFloat16Type::PyObj::py_type,
            'V',  // kind
            'f',  // type
            '=',  // byteorder
...
#define REGISTER_INT_CASTS(bits)                                                 \
    REGISTER_CAST(                                                               \
            npy_int##bits, BFloat16Type, PyArray_DescrFromType(NPY_INT##bits),   \
            BFloat16Type::npy_typenum, 1)                                        \
    REGISTER_CAST(BFloat16Type, npy_int##bits, &descr, NPY_INT##bits, 0)         \
    REGISTER_CAST(                                                               \
            npy_uint##bits, BFloat16Type, PyArray_DescrFromType(NPY_UINT##bits), \
            BFloat16Type::npy_typenum, 1)                                        \
    ...
    REGISTER_CAST(
            float, BFloat16Type, PyArray_DescrFromType(NPY_FLOAT),
            BFloat16Type::npy_typenum, 0)
    REGISTER_CAST(BFloat16Type, double, &descr, NPY_DOUBLE, 1)
    REGISTER_CAST(
            double, BFloat16Type, PyArray_DescrFromType(NPY_DOUBLE),
            BFloat16Type::npy_typenum, 0)
    return true;
}
...
void mgb::init_npy_num_bfloat16(py::module m) {
    ...
    m.add_object(
            "pybfloat16", reinterpret_cast<PyObject*>(&BFloat16Type::PyObj::py_type));
    m.add_object(
            "bfloat16",
            reinterpret_cast<PyObject*>(PyArray_DescrFromType(npy_num_bfloat16())));
}
