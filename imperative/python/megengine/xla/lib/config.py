...
import sys
import threading
from typing import Any, Callable, Hashable, Iterator, List, NamedTuple, Optional
...
class Config:
    ...
    def add_option(
        self, name, default, opt_type, meta_args, meta_kwargs, update_hook=None
    ):
    ...
    def DEFINE_enum(self, name, default, *args, **kwargs):
        update_hook = kwargs.pop("update_hook", None)
        self.add_option(name, default, "enum", args, kwargs, update_hook=update_hook)
    ...
    def define_bool_state(
        ...
        *,
        ...
        name = name.lower()
        self.DEFINE_bool(
            name, bool_env(name.upper(), default), help, update_hook=update_global_hook
        )
    ...
    def define_int_state(
        ...
        def validate(new_val):
            if new_val is not None and not isinstance(new_val, int):
                raise ValueError(
                    f"new int config value must be None or of type int, "
                    f"got {new_val} of type {type(new_val)}"
                )
    ...
    def define_string_state(
        ...
        def validate(new_val):
            if new_val is not None and not isinstance(new_val, str):
                raise ValueError(
                    f"new string config value must be None or of type str,"
                    f" got {new_val} of type {type(new_val)}."
                )

        return self.define_string_or_object_state(
            name, default, help, update_global_hook, update_thread_local_hook, validate
        )
...
#     upgrade=True,
#     help=(
#         "Enables an internal upgrade that allows one to define custom "
...
#     help=(
#         "Select the default PRNG implementation, used when one is not "
...
#         "in some cases."
#     ),
#     update_global_hook=lambda val: _update_global_jit_state(threefry_partitionable=val),
...
#     ),
#     update_global_hook=lambda val: _update_global_jit_state(numpy_dtype_promotion=val),
#     update_thread_local_hook=lambda val: update_thread_local_jit_state(
#         numpy_dtype_promotion=val
#     ),
# )
...
numpy_rank_promotion = config.define_enum_state(
    ...
    update_global_hook=lambda val: _update_global_jit_state(numpy_rank_promotion=val),
    update_thread_local_hook=lambda val: update_thread_local_jit_state(
        numpy_rank_promotion=val
    ),
)
...
config.define_bool_state(
    ...
    update_global_hook=lambda val: _update_global_jit_state(dynamic_shapes=val),
    update_thread_local_hook=lambda val: update_thread_local_jit_state(
        dynamic_shapes=val
    ),
)
