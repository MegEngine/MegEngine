...
public:
    static TimeSpec now() {
        ...
        auto t = clock();
        mgb_assert(t > 0);
        long long sec = t / CLOCKS_PER_SEC,
...
class timer_impl::RealTimeTrait {
public:
    static TimeSpec now() { return orwl_gettime(); }
};
...
public:
    static TimeSpec now() {
        ...
        if (mgb_unlikely(gettimeofday(&tv, nullptr))) {
            mgb_throw(SystemError, "gettimeofday failed: %s", strerror(errno));
        }
...
std::string TimeSpec::to_string() const {
    return ssprintf(
            "%lld.%09lld", static_cast<long long>(sec), static_cast<long long>(nsec));
}
