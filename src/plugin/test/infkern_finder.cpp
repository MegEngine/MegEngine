...
protected:
    ...
    void run(SymbolVar y, int tid) {
        ...
        ASSERT_EQ(expected_bad_opr, bad_opr) << ssprintf(
                "get opr: %s{%s}", bad_opr->cname(), bad_opr->dyn_typeinfo()->name);
        ASSERT_FALSE(finder->get_input_values(expected_bad_opr->id()).empty());
    }
};
...
TEST_F(TestInfkernFinder, UnusedVar) {
    ...
    auto x = opr::Host2DeviceCopy::make(*cg, host_x), tshp = x.make_scalar(3),
         xrshp = x.reshape(tshp), y = xrshp + 2;
    HostTensorND host_y;
    ...
    ASSERT_THROW(
            finder.get_input_values(tshp.node()->owner_opr()->id()), MegBrainError);
}
...
TEST_F(TestInfkernFinder, MultiCompile) {
    ...
    cg->options().graph_opt_level = 0;
    InfkernFinder finder{cg.get(), true};
    auto x = opr::Host2DeviceCopy::make(*cg, host_x), y0 = x + 2, y1 = x + 3;
...
TEST_F(TestInfkernFinder, InSubgraph) {
    auto loop_cb = [&](opr::Loop::Desc& desc) {
        auto xi = desc.add_input(x), y = make_callback(xi);
