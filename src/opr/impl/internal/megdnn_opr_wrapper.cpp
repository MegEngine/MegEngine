...
DeviceTensorND intl::get_temp_tensor(
...
class WorkspaceLimitGetter::Impl final : public UserDataContainer::UserData {
    ...
    void print_log() {
        ...
        std::string msg(
                "determined workspace size limit upper bound "
                "and reallocate static memory; bounds(MiB):");
        for (auto&& i : log_msg_vec) {
            msg.push_back(' ');
            msg.append(ssprintf("%s=%.2f", i.first.c_str(), i.second / 1024.0 / 1024));
        }
    ...
    void on_static_mem_alloc(const cg::event::StaticMemAlloc& ev) {
        ...
        *ev.need_realloc = true;
        ...
        free += m_first_run_var->owner_graph()->get_device_memory_size(ev.comp_node);
...
public:
    ...
    VarNode* first_run_var() const { return m_first_run_var; }

    bool is_prealloc_run() const { return !m_first_alloc_finished; }
};
MGB_TYPEINFO_OBJ_IMPL(WorkspaceLimitGetter::Impl);

WorkspaceLimitGetter::Impl* WorkspaceLimitGetter::get_impl(ComputingGraph* graph) {
...
VarNode* WorkspaceLimitGetter::register_to_graph(ComputingGraph* graph) {
    ...
    auto maker = [graph]() { return std::make_shared<Impl>(graph); };
    return graph->options()
            .user_data.get_user_data_or_create<Impl>(maker)
            ->first_run_var();
}
...
/* ================== MegDNNGraphDep ================== */
MegDNNGraphDep::MegDNNGraphDep(std::unique_ptr<megdnn::OperatorBase> opr) noexcept
        : m_opr{std::move(opr)} {}
...
void mixin::WorkspaceSizeInfer::mixin_init_output_static_infer_desc_workspace(
    ...
    for (size_t i = 0; i + 1 < opr.output().size(); ++i) {
        ...
        mgb_assert(
                mgr.get_infer_type(ovar).shape != InferType::NO_DESC,
    ...
    if (need_limit) {
        auto var = intl::WorkspaceLimitGetter::register_to_graph(opr.owner_graph());
        if (var) {
            deps.push_back({var, DepType::VALUE});
        }
    }
    mgr.register_shape_infer(out_wksp, {SourceType::DEP, deps, infer_workspace});
}
...
void MegDNNOprHolder::set_megdnn_opr(std::unique_ptr<megdnn::OperatorBase> self) {
...
void MegDNNOprHolderBwdStaticInfer::mixin_init_output_static_infer_desc_bwd(
    ...
    mgr.register_shape_infer(
            self.output(0), ShapeInferDesc::make_identity(self.input(m_oshp_idx)));
}

void MegDNNOprHolderBwdStaticInfer::mixin_init_output_dtype(OperatorNodeBase& self) {
    mgb_assert(m_oshp_idx != BAD_OSHP_IDX);
