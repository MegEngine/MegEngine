...
TEST(TestOprDNN, SlidingWindowTranspose) {
    ...
    unsigned long oh = (ih + 2 * param.pad_h - param.dilate_h * (param.window_h - 1) -
                        1) / param.stride_h +
                       1;
    unsigned long ow = (iw + 2 * param.pad_w - param.dilate_w * (param.window_w - 1) -
                        1) / param.stride_w +
                       1;
    param.out_h = ih;
    param.out_w = iw;
    ...
    auto fwd = [&](Checker::NumOutArray& dest, Checker::NumInpArray inp) {
        ...
        dest[0].dtype(dtype::Float32())
                ...
                .resize(dest_layout);
        opr->exec(
                inp[0]->as_megdnn(), dest[0].as_megdnn(),
    ...
    checker.run({TensorShape{2, 3, oh, ow, param.window_h, param.window_w}}, opt)
