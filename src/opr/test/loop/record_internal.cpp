...
TEST(TestOprLoopRecordInternal, ImpureOprRNG) {
    ...
    HostTensorGenerator<dtype::Float32, RandomDistribution::UNIFORM> genx{1e-3, 1.5};

    auto host_x = genx({SIZE}), host_loss_p = gen({SIZE});
    ...
    auto loss = opr::Dot::make(y, opr::Host2DeviceCopy::make(*graph, host_loss_p)),
         gx = cg::grad(loss, x);
