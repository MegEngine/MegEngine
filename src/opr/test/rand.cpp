...
struct BasicStat {
    ...
    static BasicStat make(const float* ptr, size_t size, double mean_expect = 0) {
...
TEST(TestOprRand, Gamma) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        float mean = a * b;
        float std = a * (b * b);
        auto stat = BasicStat::make(host_y.ptr<float>() + 2000000 * i, 2000000, mean);
        ASSERT_LT(fabs(stat.mean - mean), 0.01);
...
TEST(TestOprRand, Poisson) {
    ...
    for (int i = 0; i < 5; ++i) {
        float lambda = i + 1;
        auto stat = BasicStat::make(host_y.ptr<float>() + 200000 * i, 200000, lambda);
...
TEST(TestOprRand, ShuffleForward) {
    auto run = [&](TensorShape shape) {
        std::shared_ptr<HostTensorND> src_host(
        ...
        auto func = graph->compile(
                {make_callback_copy(rec[0], host_y),
...
TEST(TestOprRand, UniformReprod) {
    ...
    auto graph = ComputingGraph::make();
    auto shp =
            cg::var_from_tensor_shape(*graph, {CompNode::load("xpu0")}, "shp0", {SIZE});
