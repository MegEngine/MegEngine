...
template <typename T, typename U>
void fill_exponential(Xoroshiro128plus* rng, U* dst, U* scale, size_t size) {
    for (size_t i = 0; i < size; ++i) {
        T scl = static_cast<T>(scale[i]);
        T u = uniform_sample<T>(rng);
        dst[i] = static_cast<U>(-scl * std::log(u));
    }
}
...
void ExponentialRNGImpl::exec(
        _megdnn_tensor_in scale, _megdnn_tensor_inout dst, _megdnn_workspace workspace) {
    check_exec(scale.layout, dst.layout, workspace.size);
    auto size = dst.layout.total_nr_elems();
    auto prng = &m_rng.ensure_seed(m_param.seed);
    switch (dst.layout.dtype.enumv()) {
#define cb(_dt)                                                                        \
    case DTypeTrait<_dt>::enumv: {                                                     \
        using ctype = DTypeTrait<_dt>::ctype;                                          \
        MEGDNN_DISPATCH_CPU_KERN_OPR({                                                 \
            fill_exponential<float>(prng, dst.ptr<ctype>(), scale.ptr<ctype>(), size); \
        };);                                                                           \
        return;                                                                        \
    }
        MEGDNN_FOREACH_COMPUTING_DTYPE_FLOAT(cb)
