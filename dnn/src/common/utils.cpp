...
bool megdnn::get_next_addr(size_t* idx, const size_t* shp, size_t n, size_t stride) {
    auto errmsg = [&]() {
        std::string res;
        res.append("idx={");
...
size_t megdnn::infer_conv_shape(
        size_t inp, size_t flt, size_t stride, size_t pad, bool is_floor) {
    megdnn_assert(
            inp + 2 * pad >= flt, "input=%zu padding=%zu filter=%zu", inp, pad, flt);
    if (is_floor) {
...
void megdnn::infer_conv_shape2d(
        size_t ih, size_t iw, size_t fh, size_t fw, size_t sh, size_t sw, size_t ph,
        size_t pw, size_t& oh, size_t& ow, bool is_floor) {
...
bool megdnn::is_nhwc_contig_wc(const TensorLayout& layout) {
    return layout.ndim == 4 && (layout.stride[3] == 1 || layout.shape[3] == 1) &&
...
megcoreDeviceHandle_t megdnn::get_device_handle(Handle* handle) {
    ...
    megdnn_throw_if(status != megcoreSuccess, megdnn_error, "get device handle error!");
    return dev_handle;
}
...
template <>
uint8_t megdnn::convert<dt_quint4, uint8_t>(dt_quint4 src, uint8_t dst, size_t offset) {
    uint8_t _src = std::min(src.as_uint8(), DTypeTrait<dtype::Quantized4Asymm>::max());
    if (offset == 0) {
        _src &= 0xF;
        dst &= 0xF0;
        dst |= _src;
...
template <>
dt_quint4 megdnn::convert<uint8_t, dt_quint4>(
        uint8_t src, dt_quint4 dst, size_t offset) {
    src >>= (offset << 2);
    src &= 0xF;
    dst = dt_quint4(src);
    return dst;
...
template <>
int8_t megdnn::convert<dt_qint4, int8_t>(dt_qint4 src, int8_t dst, size_t offset) {
    int8_t _src = std::max(
...
template <>
dt_qint4 megdnn::convert<int8_t, dt_qint4>(int8_t src, dt_qint4 dst, size_t offset) {
    src <<= (4 - (offset << 2));
    src >>= 4;
    dst = dt_qint4(src);
    return dst;
...
size_t& CpuNDRange::operator[](size_t idx) {
    megdnn_assert(
            idx < m_dimension, "invalid index: %zu expected < %zu", idx, m_dimension);
    return m_dim[idx];
}

bool megdnn::check_bias_share_in_channel(
    ...
    if (format == param::ConvBias::Format::NCHW ||
        ...
        share_in_channel =
                (bias.ndim == 4 && bias[0] == 1 && bias[2] == 1 && bias[3] == 1);
    } else if (
            format == param::ConvBias::Format::NHWC ||
            format == param::ConvBias::Format::NCHW4_NHWC) {
        share_in_channel =
                (bias.ndim == 4 && bias[0] == 1 && bias[1] == 1 && bias[2] == 1);
    } else if (
            format == param::ConvBias::Format::NCHW4 ||
        ...
        share_in_channel =
                (bias.ndim == 5 && bias[0] == 1 && bias[2] == 1 && bias[3] == 1);
    } else if (format == param::ConvBias::Format::NHWCD4) {
        share_in_channel =
                (bias.ndim == 5 && bias[0] == 1 && bias[1] == 1 && bias[3] == 1);
    } else {
        megdnn_assert(format == param::ConvBias::Format::CHWN4);
        share_in_channel =
                (bias.ndim == 5 && bias[1] == 1 && bias[2] == 1 && bias[3] == 1);
    }
