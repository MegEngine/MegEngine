...
void update_tile_repeat_state(
    ...
    if (nr_reduces == 1) {
        ...
        if (state == 0) {
            ...
            megdnn_assert(
                    current == workspace0 || current == workspace1,
...
#define INST(T)                                                      \
    template void tile_or_repeat_single_axis<T>(                     \
            const T*, T*, const size_t, const size_t, const size_t); \
    template void init_tile_repeat_state<T>(                         \
            const T*, T*, T*, T*, T*&, T*&, size_t&, size_t);        \
    template void update_tile_repeat_state<T>(                       \
            const T*, T*, T*, T*, T*&, T*&, size_t&, size_t);
