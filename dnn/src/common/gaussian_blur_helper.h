...
template <typename T>
inline static void createGaussianKernels(
        Mat<T>& kx, Mat<T>& ky, Size ksize, double sigma1, double sigma2) {
    if (sigma2 <= 0)
        sigma2 = sigma1;

    if (ksize.cols() <= 0 && sigma1 > 0) {
        double num = sigma1 * (std::is_same<T, unsigned char>::value ? 3 : 4) * 2 + 1;
        num = (int)(num + (num >= 0 ? 0.5 : -0.5));
        ksize.cols() = ((int)num) | 1;
    }
    if (ksize.rows() <= 0 && sigma2 > 0) {
        double num = sigma2 * (std::is_same<T, unsigned char>::value ? 3 : 4) * 2 + 1;
        num = (int)(num + (num >= 0 ? 0.5 : -0.5));
        ksize.rows() = ((int)num) | 1;
    }

    megdnn_assert(
            ksize.cols() > 0 && ksize.cols() % 2 == 1 && ksize.rows() > 0 &&
            ksize.rows() % 2 == 1);
