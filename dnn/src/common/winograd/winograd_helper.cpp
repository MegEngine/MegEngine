...
struct Getter<
    ...
    otype zp;
    Getter(DType dtype) { zp = dtype.param<dtype::Quantized8Asymm>().zero_point; }
...
template <typename ctype, typename otype>
struct OutputGetter<
        ctype, otype, typename std::enable_if_t<std::is_same<otype, int8_t>::value>> {
    DType dtype;
...
template <typename ctype, typename otype>
struct OutputGetter<
        ctype, otype, typename std::enable_if_t<std::is_same<otype, uint8_t>::value>> {
    DType dtype;
    OutputGetter(DType dtype) : dtype{dtype} {}
    otype operator()(float item) {
...
struct FilterVisitor {
    ...
    size_t put(size_t alpha, size_t oc, size_t ic, size_t h, size_t w) {
        ...
        return (h * alpha + w) * OCB * ICB * matmul_pack_size * matmul_pack_size +
               ocb * ICB * matmul_pack_size * matmul_pack_size +
               icb * matmul_pack_size * matmul_pack_size + ic_pack * matmul_pack_size +
               oc_pack;
    }
};
...
struct InputVisitor {
    ...
    size_t get(
            size_t /*alpha*/, size_t ic, size_t IH, size_t IW, size_t ih, size_t iw) {
        constexpr size_t input_pack_size = layout_pack_size(layout);
        size_t icb_layout = ic / input_pack_size;
        size_t ic_layout = ic % input_pack_size;

        return (icb_layout * IH * IW + ih * IW + iw) * input_pack_size + ic_layout;
    }

    size_t put(
            size_t alpha, size_t ic, size_t nr_units_in_tile, size_t unit_idx, size_t h,
            size_t w) {
        if (format == param::MatrixMul::Format::DEFAULT) {
        ...
        return (h * alpha + w) * ICB * nr_units_in_tile * matmul_pack_size +
               icb * nr_units_in_tile * matmul_pack_size + unit_idx * matmul_pack_size +
               ic_pack;
    }
};
...
struct OutputVisitor {
    ...
    size_t get(
            size_t alpha, size_t oc_index, size_t oc, size_t nr_units_in_tile,
            size_t unit_idx, size_t h, size_t w) {
        if (format == param::MatrixMul::Format::DEFAULT) {
            return (h * alpha + w) * nr_units_in_tile * OC + unit_idx * OC + oc_index;
        }
        size_t matmul_pack_size = MatrixMulForward::pack_size(format);
        ...
        return (h * alpha + w) * OCB * nr_units_in_tile * matmul_pack_size +
               ocb * nr_units_in_tile * matmul_pack_size + unit_idx * matmul_pack_size +
               oc_pack;
    }
    ...
    size_t put(size_t oc, size_t OH, size_t OW, size_t oh, size_t ow) {
        ...
        return (oc / input_pack_size * OH * OW + oh * OW + ow) * input_pack_size +
               oc_layout;
    }
};
...
void StrategyHelper<
        ...
        format>::
        filter(const ctype* filter, input_filter_compute_type* filter_transform_buf,
               input_filter_compute_type* transform_mid_buf, size_t OC, size_t IC,
               size_t oc_start, size_t oc_end, size_t m, size_t r,
               const std::vector<float>& interp_points, DType dtype, float rescale) {
    size_t alpha = m + r - 1;
    WinogradCoeff<input_filter_compute_type> winograd_coeff(m, r, interp_points);
    ...
    for (size_t oc = oc_start; oc < oc_end; oc++) {
        rep(ic, IC) {
            ...
            megdnn::naive::run_matrix_mul_tpl<
                    input_filter_compute_type, input_filter_compute_type, false, false>(
                    winograd_coeff.G(rescale).data(), mid_buf1, mid_buf2, alpha, r, r,
                    r, r, r, dtype, dtype);
            /* dst = Matmul(tmp, G^T) */
            megdnn::naive::run_matrix_mul_tpl<
                    input_filter_compute_type, input_filter_compute_type, false, true>(
                    mid_buf2, winograd_coeff.G(rescale).data(), mid_buf1, alpha, alpha,
                    r, r, r, alpha, dtype, dtype);
...
void StrategyHelper<
        ctype, dst_type, input_filter_compute_type, output_compute_type, layout,
        format>::
        input(const ctype* input, input_filter_compute_type* input_transform_buf,
    ...
    rep(i, alpha) rep(j, alpha) {
        ...
        if (ih >= 0 && ih < (int)IH && iw >= 0 && iw < (int)IW) {
            mid_buf1[i * alpha + j] =
                    getter(input[intput_visitor.get(alpha, ic, IH, IW, ih, iw)]);
        }
    }

    megdnn::naive::run_matrix_mul_tpl<
            input_filter_compute_type, input_filter_compute_type, true, false>(
            winograd_coeff.B(rescale).data(), mid_buf1, mid_buf2, alpha, alpha, alpha,
            alpha, alpha, alpha, dtype, dtype);
    megdnn::naive::run_matrix_mul_tpl<
            input_filter_compute_type, input_filter_compute_type, false, false>(
            mid_buf2, winograd_coeff.B(rescale).data(), mid_buf1, alpha, alpha, alpha,
            alpha, alpha, alpha, dtype, dtype);

    rep(i, alpha) rep(j, alpha) {
        input_transform_buf[intput_visitor.put(
                alpha, ic, nr_units_in_tile, unit_idx, i, j)] = mid_buf1[i * alpha + j];
    }
}
...
void StrategyHelper<
        ...
        output(const output_compute_type* output_transform_buf,
               ...
               float input_filter_scale, float input_filter_rescale, float rescale) {
    size_t alpha = m + r - 1;
    winograd::WinogradCoeff<output_compute_type> winograd_coeff(m, r, interp_points);
    ...
    rep(i, alpha) rep(j, alpha) {
        mid_buf1[i * alpha + j] = output_transform_buf[output_visitor.get(
                alpha, oc_index, oc, nr_units_in_tile, unit_idx, i, j)];
    }
    /* A[alpha*m] M[alpha*alpha] */
    megdnn::naive::run_matrix_mul_tpl<
            output_compute_type, output_compute_type, true, false>(
            winograd_coeff.A(rescale).data(), mid_buf1, mid_buf2, m, alpha, alpha, m,
            alpha, alpha, dtype, dtype);
    megdnn::naive::run_matrix_mul_tpl<
            output_compute_type, output_compute_type, false, false>(
            mid_buf2, winograd_coeff.A(rescale).data(), mid_buf1, m, m, alpha, alpha, m,
            m, dtype, dtype);
    ...
    rep(i, m) rep(j, m) {
        ...
        if (oh < OH && ow < OW) {
            ...
            if (bmode == BiasMode::BROADCAST_CHANNEL_BIAS) {
                val += bias[oc] * input_filter_rescale * input_filter_rescale;
            ...
            val = val * input_filter_scale /
                  (input_filter_rescale * input_filter_rescale * rescale * rescale);
...
#define INST(_ctype, _dst_type, _input_filter_compute_type, _output_compute_type) \
    template class StrategyHelper<                                                \
            _ctype, _dst_type, _input_filter_compute_type, _output_compute_type>;
...
#define INST(                                                                        \
        _ctype, _dst_type, _input_filter_compute_type, _output_compute_type, layout) \
    template class StrategyHelper<                                                   \
            _ctype, _dst_type, _input_filter_compute_type, _output_compute_type,     \
            layout, param::MatrixMul::Format::MK4>;
...
#define INST(                                                                        \
    ...
    template class StrategyHelper<                                                   \
            _ctype, _dst_type, _input_filter_compute_type, _output_compute_type,     \
            layout, param::MatrixMul::Format::MK8>;
...
DNN_INC_FLOAT16(INST(
        dt_float16, dt_float16, dt_float16, dt_float16, param::ConvBias::Format::NCHW))
#undef INST
}  // namespace winograd
}  // namespace megdnn
