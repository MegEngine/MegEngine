...
struct PostProcess {
    static void run(
            ...
            megdnn::param::ConvBias::NonlineMode nonlineMode, DType bias_type,
...
#define CALL_BINARY_BROADCAST(_op, _simd_type)                                       \
    thin_function<void(                                                              \
            const ctype*, const ctype*, dtype*, DType, DType, DType, size_t, size_t, \
            size_t)>                                                                 \
            run = OpCallerBinary<                                                    \
                    _op<_simd_type, ctype, dtype>, _simd_type,                       \
                    megdnn::x86::BcastType::VEC_BCAST101>::run;                      \
    run(static_cast<ctype*>(conv_dst_ptr), static_cast<ctype*>(bias_ptr),            \
        reinterpret_cast<dtype*>(dst_ptr), bias_type, bias_type, dst_type, N, OC,    \
        OH* OW);
...
struct PostProcess<ctype, dtype, megdnn::PostprocessMode::QUANTIZED> {
    static void run(
            void* conv_dst_ptr, void* bias_ptr, void* dst_ptr,
            megdnn::ConvBiasForward::BiasMode bias_mode,
            megdnn::param::ConvBiasV0::NonlineMode nonlineMode, DType bias_type,
            DType dst_type, size_t N, size_t OC, size_t OH, size_t OW,
            size_t pack_oc_size = 1) {
        MEGDNN_MARK_USED_VAR(pack_oc_size);
        megdnn_assert(pack_oc_size == 1, "PostProcess only support nchw nchw in x86");
        megdnn::param::Elemwise::Mode elem_mode = megdnn::param::Elemwise::Mode::ADD;
        if (bias_mode != megdnn::ConvBiasForward::BiasMode::NO_BIAS) {
            switch (nonlineMode) {
                BIAS_CASE(RELU);
...
#define CALL_BINARY(_op, _simd_type)                                          \
    thin_function<void(                                                       \
            const ctype*, const ctype*, dtype*, DType, DType, DType, size_t)> \
            run = OpCallerBinary<                                             \
                    _op<_simd_type, ctype, dtype>, _simd_type,                \
                    megdnn::x86::BcastType::VEC_VEC>::run;                    \
    run(static_cast<ctype*>(conv_dst_ptr), static_cast<ctype*>(bias_ptr),     \
    ...
    switch (_nonline_mode) {                                                           \
        case param::ConvBias::NonlineMode::IDENTITY:                                   \
            MIDOUT_BEGIN(_midout_tag, _bias_id, 0) {                                   \
                cb(_bmode,                                                             \
                   NoneOp<_simd_type MEGDNN_COMMA _src_type MEGDNN_COMMA _dst_type>,   \
                   __VA_ARGS__);                                                       \
            }                                                                          \
            MIDOUT_END();                                                              \
            break;                                                                     \
        case param::ConvBias::NonlineMode::RELU: {                                     \
            MIDOUT_BEGIN(_midout_tag, _bias_id, 1) {                                   \
                cb(_bmode,                                                             \
                   ReluOp<_simd_type MEGDNN_COMMA _src_type MEGDNN_COMMA _dst_type>,   \
                   __VA_ARGS__);                                                       \
            }                                                                          \
            MIDOUT_END();                                                              \
            break;                                                                     \
        ...
        case param::ConvBias::NonlineMode::SIGMOID: {                                  \
            MIDOUT_BEGIN(_midout_tag, _bias_id, 2) {                                   \
                cb(_bmode,                                                             \
                   SigmoidOp<                                                          \
                           _simd_type MEGDNN_COMMA _src_type MEGDNN_COMMA _dst_type>,  \
                   __VA_ARGS__);                                                       \
            }                                                                          \
            MIDOUT_END();                                                              \
            break;                                                                     \
        ...
        case param::ConvBias::NonlineMode::H_SWISH: {                                  \
            MIDOUT_BEGIN(_midout_tag, _bias_id, 3) {                                   \
                cb(_bmode,                                                             \
                   HSwishOp<_simd_type MEGDNN_COMMA _src_type MEGDNN_COMMA _dst_type>, \
                   __VA_ARGS__);                                                       \
            }                                                                          \
            MIDOUT_END();                                                              \
            break;                                                                     \
...
#define DISPATCH_CONV_WINOGRAD_BIAS(                                                   \
        _midout_tag, cb, _simd_type, _src_type, _dst_type, _bmode, _nonline_mode, ...) \
    switch (_bmode) {                                                                  \
        case BiasMode::BIAS: {                                                         \
