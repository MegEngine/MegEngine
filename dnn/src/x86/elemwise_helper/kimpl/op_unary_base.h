...
#define OP_BASE(_simd_type, _simd_target, _simd_data_type, _func_prefix)              \
    ...
    struct UnaryOpBase<_simd_type, dt_quint8, dt_quint8>                              \
        ...
        UnaryOpBase(                                                                  \
                float src_scale, float dst_scale, uint8_t src_zp, uint8_t dst_zp) {   \
            init(src_scale, dst_scale, src_zp, dst_zp);                               \
        }                                                                             \
    };
...
#define OP_BASE(_simd_type, _simd_target, _simd_data_type, _func_prefix)           \
    ...
    struct UnaryOpBase<_simd_type, dt_float32, dt_quint8>                          \
        ...
        UnaryOpBase(float dst_scale, uint8_t dst_zp) { init(dst_scale, dst_zp); }  \
    };
OP_BASE(SIMDType::SSE4_2, "sse4.2", __m128, mm)
OP_BASE(SIMDType::AVX2, "avx2", __m256, mm256)
#undef OP_BASE
...
#define OP_BASE(_simd_type, _simd_target, _simd_data_type, _func_prefix)              \
    template <>                                                                       \
    struct UnaryOpBase<_simd_type, dt_qint8, dt_qint8> : OpBase<dt_qint8, dt_qint8> { \
        using OpBase::OpBase;                                                         \
        using src_ctype = dt_qint8;                                                   \
...
template <>
struct UnaryOpBase<SIMDType::NONE, dt_qint32, dt_qint8> : OpBase<dt_qint32, dt_qint8> {
    using OpBase::OpBase;
    using src_ctype = dt_qint32;
    using dst_ctype = dt_qint8;
    float scale, scale_src, scale_dst;
...
#define OP_BASE(_simd_type, _simd_target, _simd_data_type, _func_prefix)           \
    ...
    struct UnaryOpBase<_simd_type, dt_quint8, dt_float32>                          \
        ...
        UnaryOpBase(float src_scale, uint8_t src_zp) { init(src_scale, src_zp); }  \
    };
...
#define OPERATOR_UNARY_QUINT8(_func_prefix)                                           \
    ...
    auto vitem0 = _##_func_prefix##_mul_ps(                                           \
            _##_func_prefix##_cvtepi32_ps(_##_func_prefix##_sub_epi32(val_0, v_szp)), \
            v_scale_src);                                                             \
    auto vitem1 = _##_func_prefix##_mul_ps(                                           \
            _##_func_prefix##_cvtepi32_ps(_##_func_prefix##_sub_epi32(val_1, v_szp)), \
            v_scale_src);                                                             \
    auto vitem2 = _##_func_prefix##_mul_ps(                                           \
            _##_func_prefix##_cvtepi32_ps(_##_func_prefix##_sub_epi32(val_2, v_szp)), \
            v_scale_src);                                                             \
    auto vitem3 = _##_func_prefix##_mul_ps(                                           \
            _##_func_prefix##_cvtepi32_ps(_##_func_prefix##_sub_epi32(val_3, v_szp)), \
            v_scale_src);                                                             \
    vitem0 = op(vitem0);                                                              \
    vitem1 = op(vitem1);                                                              \
    vitem2 = op(vitem2);                                                              \
...
struct UnaryQuantizationOp<SIMDType::SSE4_2, dt_qint8, dt_qint8, Op>
    ...
    void operator()(const __m128ix2& vsrc, dt_qint8* dst) const {
        ...
        _mm_storeu_si128(
                reinterpret_cast<__m128i*>(dst + SIMD_WIDTH), operator()(vsrc.val[1]));
    }
    MEGDNN_ATTRIBUTE_TARGET("sse4.2")
...
struct UnaryQuantizationOp<SIMDType::AVX2, dt_qint8, dt_qint8, Op>
    ...
    void operator()(const __m256ix2& vsrc, dt_qint8* dst) const {
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(dst), operator()(vsrc.val[0]));
        _mm256_storeu_si256(
                reinterpret_cast<__m256i*>(dst + SIMD_WIDTH), operator()(vsrc.val[1]));
    }
    MEGDNN_ATTRIBUTE_TARGET("avx2")
...
struct UnaryQuantizationOp<SIMDType::SSE4_2, dt_qint32, dt_qint8, Op>
    ...
    int64_t operator()(const __m128ix2& vsrc) const {
        auto vitem0 = _mm_mul_ps(_mm_cvtepi32_ps(vsrc.val[0]), this->vscale_src);
        auto vitem1 = _mm_mul_ps(_mm_cvtepi32_ps(vsrc.val[1]), this->vscale_src);
        vitem0 = op(vitem0);
        vitem1 = op(vitem1);
...
struct UnaryQuantizationOp<SIMDType::AVX2, dt_qint32, dt_qint8, Op>
    ...
    __m128i operator()(const __m256ix2& vsrc) const {
        ...
        auto vitem0 = _mm256_mul_ps(_mm256_cvtepi32_ps(vsrc.val[0]), v_scale_src);
        auto vitem1 = _mm256_mul_ps(_mm256_cvtepi32_ps(vsrc.val[1]), v_scale_src);
        vitem0 = op(vitem0);
        vitem1 = op(vitem1);
