...
#define DISPATCH_MODE(_src_dt, _dst_dt, _simd_type)                              \
    switch (mode) {                                                              \
        DISPATCH_SINGLE_MODE(                                                    \
                _src_dt, _dst_dt, Elemwise::Mode::RELU, ReluOp, _simd_type)      \
        DISPATCH_SINGLE_MODE(                                                    \
                _src_dt, _dst_dt, Elemwise::Mode::H_SWISH, HSwishOp, _simd_type) \
        default:                                                                 \
            break;                                                               \
    }
...
#define DISPATCH_QUANTIZED_MODE(_src_dt, _dst_dt, _simd_type)                          \
    switch (mode) {                                                                    \
        DISPATCH_SINGLE_MODE(                                                          \
                _src_dt, _dst_dt, Elemwise::Mode::RELU, ReluOp, _simd_type)            \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::ABS, AbsOp, _simd_type) \
        DISPATCH_SINGLE_MODE(                                                          \
                _src_dt, _dst_dt, Elemwise::Mode::SIGMOID, SigmoidOp, _simd_type)      \
        DISPATCH_SINGLE_MODE(                                                          \
                _src_dt, _dst_dt, Elemwise::Mode::FAST_TANH, FastTanhOp, _simd_type)   \
        DISPATCH_SINGLE_MODE(                                                          \
                _src_dt, _dst_dt, Elemwise::Mode::H_SWISH, HSwishOp, _simd_type)       \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::EXP, ExpOp, _simd_type) \
        default:                                                                       \
            break;                                                                     \
    }
...
#define DISPATCH_DATA_TYPE(_simd_type)                                              \
    if (param[0].layout.dtype.enumv() == DTypeEnum::QuantizedS8 &&                  \
        ...
        DISPATCH_QUANTIZED_MODE(dtype::QuantizedS8, dtype::QuantizedS8, _simd_type) \
    } else if (                                                                     \
        ...
        DISPATCH_QUANTIZED_MODE(                                                    \
                dtype::Quantized8Asymm, dtype::Quantized8Asymm, _simd_type)         \
    } else if (                                                                     \
...
#define DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, _mode, _op, _simd_type)                 \
    case _mode: {                                                                      \
        ...
        thin_function<void(const src_ctype*, dst_ctype*, DType, DType, size_t)> run =  \
                OpCallerUnary<_op<_simd_type, src_ctype, dst_ctype>, _simd_type>::run; \
...
#define DISPATCH_MODE(_src_dt, _dst_dt, _simd_type)                                    \
    switch (mode) {                                                                    \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::ADD, AddOp, _simd_type) \
        DISPATCH_SINGLE_MODE(                                                          \
...
#define DISPATCH_QUANTIZED_MODE(_src_dt, _dst_dt, _simd_type)                          \
    switch (mode) {                                                                    \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::ADD, AddOp, _simd_type) \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::MIN, MinOp, _simd_type) \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::MAX, MaxOp, _simd_type) \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::SUB, SubOp, _simd_type) \
        DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, Elemwise::Mode::MUL, MulOp, _simd_type) \
        DISPATCH_SINGLE_MODE(                                                          \
                _src_dt, _dst_dt, Elemwise::Mode::FUSE_ADD_RELU, FuseAddReluOp,        \
                _simd_type)                                                            \
        DISPATCH_SINGLE_MODE(                                                          \
...
#define DISPATCH_SINGLE_MODE(_src_dt, _dst_dt, _mode, _op, _simd_type)               \
    case _mode: {                                                                    \
        ...
        thin_function<void(                                                          \
                ...
                size_t)>                                                             \
                run = OpCallerBinary<                                                \
                        _op<_simd_type, src_ctype, dst_ctype>, _simd_type,           \
                        VEC_VEC>::run;                                               \
