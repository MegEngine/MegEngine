...
MIOpenCache<ConvolutionForwardImpl::AlgoBase::SizeArgs, miopenConvFwdAlgorithm_t>
...
bool ConvolutionForwardImpl::AlgoMIOpen::is_available(const SizeArgs& args) const {
...
size_t ConvolutionForwardImpl::AlgoMIOpen::get_workspace_in_bytes(
    ...
    megdnn_assert(
            ...
            "conv fwd get workspace failed: %s; info: %s", miopenGetErrorString(status),
...
void ConvolutionForwardImpl::AlgoMIOpen::exec(const ExecArgs& args) const {
    ...
    args.init_desc(D);
    auto algo =
            const_cast<ConvolutionForwardImpl::AlgoMIOpen*>(this)->find_best_algo(args);
...
void ConvolutionForwardImpl::AlgoPack::fill_miopen_algos() {
    megdnn_throw(
            "MIOpen has implemented auto-tuning in the framework, so we do not need to "
            "choose algorithms manually");
}
