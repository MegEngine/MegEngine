...
void do_max_pooling_w4x4_s2x2_float_NEON(
    ...
    for (; oh < OH && -PH + stride * oh + window <= IH; ++oh) {
        ...
        if (-PW + stride * ow + window <= IW) {
            ...
            dst[oh * OW + ow] = std::max(vget_lane_f32(t, 0), vget_lane_f32(t, 1));
        ...
        for (; ow + 1 < OW && -PW + stride * (ow + 1) + window <= IW; ow += 2) {
            ...
            dst[oh * OW + ow + 1] = std::max(vget_lane_f32(t, 0), vget_lane_f32(t, 1));
            last_hf_res = vget_lane_f32(t, 1);
        }
        for (; ow < OW; ++ow) {
            do_pxl_naive<Pooler, window>(
...
#define cb(i) \
    dst[oh * OW + ow + i] = std::max(vget_lane_s8(t, i), vget_lane_s8(t, i + 1));
...
#define cb(i) \
    dst[oh * OW + ow + i + 1] = std::max(vget_lane_s8(t, i), vget_lane_s8(t, i + 1));
            UNROLL_CALL_NOWRAPPER(7, cb)
#undef cb
            last_res = vget_lane_s8(t, 7);
        }
...
#define cb(i) \
    dst[oh * OW + ow + i] = std::max(vget_lane_u8(t, i), vget_lane_u8(t, i + 1));
            UNROLL_CALL_NOWRAPPER(7, cb)
#undef cb
...
#define cb(i) \
    dst[oh * OW + ow + i + 1] = std::max(vget_lane_u8(t, i), vget_lane_u8(t, i + 1));
            UNROLL_CALL_NOWRAPPER(7, cb)
#undef cb
            last_res = vget_lane_u8(t, 7);
        }
...
void do_max_pooling_w4x4_s2x2_float16_NEON(
    ...
    for (; oh < OH && -PH + stride * oh + window <= IH; ++oh) {
        ...
        if (-PW + stride * (ow + 2) + window <= IW) {
            ...
            dst[oh * OW + ow] = std::max(vget_lane_f16(t, 0), vget_lane_f16(t, 1));
            dst[oh * OW + ow + 1] = std::max(vget_lane_f16(t, 1), vget_lane_f16(t, 2));
            dst[oh * OW + ow + 2] = std::max(vget_lane_f16(t, 2), vget_lane_f16(t, 3));
        ...
        for (; ow + 3 < OW && -PW + stride * (ow + 3) + window <= IW; ow += 4) {
            ...
            dst[oh * OW + ow + 1] = std::max(vget_lane_f16(t, 0), vget_lane_f16(t, 1));
            dst[oh * OW + ow + 2] = std::max(vget_lane_f16(t, 1), vget_lane_f16(t, 2));
            dst[oh * OW + ow + 3] = std::max(vget_lane_f16(t, 2), vget_lane_f16(t, 3));
            last_hf_res = vget_lane_f16(t, 3);
        }
        for (; ow < OW; ++ow) {
            do_pxl_naive<Pooler, window>(
