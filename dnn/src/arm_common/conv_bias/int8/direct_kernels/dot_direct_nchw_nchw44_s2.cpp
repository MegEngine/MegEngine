...
#define cb(step)                                                                      \
    c[0][step * 2] = Func::template impl<(src_idx + step) % 4>(                       \
            c[0][step * 2], weight[0][weight_idx], src[0][(src_idx + step) / 4]);     \
    c[1][step * 2] = Func::template impl<(src_idx + step) % 4>(                       \
            c[1][step * 2], weight[1][weight_idx], src[0][(src_idx + step) / 4]);     \
    c[0][step * 2 + 1] = Func::template impl<(src_idx + step) % 4>(                   \
            c[0][step * 2 + 1], weight[0][weight_idx], src[1][(src_idx + step) / 4]); \
    c[1][step * 2 + 1] = Func::template impl<(src_idx + step) % 4>(                   \
            c[1][step * 2 + 1], weight[1][weight_idx], src[1][(src_idx + step) / 4]);
...
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 1, oc_block, ow_block, 2> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 2, oc_block, ow_block, 2> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 3, oc_block, ow_block, 2> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 5, oc_block, ow_block, 2> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
        ...
        for (int ic_idx = 0; ic_idx < ic; ic_idx += ic_step) {
            ...
            int8x16_t weight[c_dim][weight_reg];
#define cb(step)                                                                      \
    load_helper<src_reg, 0, simd_len, 2, Vld1q_s8>(src, src_ptr + step * iw, stride); \
    load_helper<weight_reg, 0, simd_len, c_dim, Vld1q_s8>(                            \
 ...
 **/
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 7, oc_block, ow_block, 2> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
...
void pack_src_int8_nchw_nchw44_dot<2>(
    ...
    rep_step(ic_idx, ic, ic_step) {
        ...
        rep(ih_idx, ih) {
            memcpy(sptr_base + pw * ic_step, sptr, sizeof(int8_t) * iw * ic_step);
