...
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 1, oc_block, ow_block, 1> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
        ...
        for (int ic_idx = 0; ic_idx < ic; ic_idx += ic_step) {
            ...
            cal_helper<0, 0, c_dim, Vdotq_laneq_s32, ow_block, stride>(c, src, weight);
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 2, oc_block, ow_block, 1> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
        ...
        for (int ic_idx = 0; ic_idx < ic; ic_idx += ic_step) {
            ...
            cal_helper<0, 1, c_dim, Vdotq_laneq_s32, ow_block, stride>(c, src, weight);
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 3, oc_block, ow_block, 1> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
        ...
        for (int ic_idx = 0; ic_idx < ic; ic_idx += ic_step) {
            ...
            cal_helper<0, 2, c_dim, Vdotq_laneq_s32, ow_block, stride>(c, src, weight);
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 5, oc_block, ow_block, 1> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
...
template <BiasMode bias_mode, typename Op, int remain_w, int oc_block, int ow_block>
struct KerNeonDotXXs2Nchw44Int8<bias_mode, Op, remain_w, 7, oc_block, ow_block, 1> {
    MEGDNN_ATTRIBUTE_TARGET("dotprod")
    static void impl(
            const int8_t* src_ptr, const int8_t* weight_ptr, const int32_t* bias_ptr,
            int8_t* dst_ptr, int ic, int ih, int iw, int ld_dst_oc, const Op& op) {
...
template <>
void pack_src_int8_nchw_nchw44_dot<1>(
        ...
        const int pad_bottom, const int ic, const int ic_stride, int8_t* temp_ptr) {
    static uint8_t reorder_idx[16] = {0, 1, 2, 3, 1, 2, 3, 4, 2, 3, 4, 5, 3, 4, 5, 6};
    ...
    rep(ic_idx, ic) {
        ...
        rep(ih_idx, ih) {
            ...
            for (int iw_idx = iw_with_pad_end; iw_idx < iw_with_pad; ++iw_idx) {
                *(sptr_base + iw_idx * pack_iw_len + 0) = *(temp_ptr + iw_idx + 0);
                *(sptr_base + iw_idx * pack_iw_len + 1) = *(temp_ptr + iw_idx + 1);
                *(sptr_base + iw_idx * pack_iw_len + 2) = *(temp_ptr + iw_idx + 2);
                *(sptr_base + iw_idx * pack_iw_len + 3) = *(temp_ptr + iw_idx + 3);
            }
...
MEGDNN_ATTRIBUTE_TARGET("dotprod")
void conv_direct_int8_nchw_nchw44_dot(
        const int8_t* src, const int8_t* filter, const int32_t* bias, int32_t* temp,
        int8_t* dst, const int oc, const int ic, const int ih, const int iw,
        const int oh, const int oh_block, const int ow, const Op& op) {
...
#define cb(step)                                                                       \
    case step:                                                                         \
        kern_big_oc_remain = KerNeonDotXXs2Nchw44Int8<                                 \
                bias_mode, Op, step, filter_size, big_oc_step, ow_step, stride>::impl; \
        kern_small_oc_remain = KerNeonDotXXs2Nchw44Int8<                               \
                bias_mode, Op, step, filter_size, oc_step, ow_step, stride>::impl;     \
        break;
        ...
        for (int oh_idx = 0; oh_idx < oh_block; oh_idx += oh_step) {
            for (int ow_idx = 0; ow_idx < ow_end; ow_idx += ow_step) {
                ...
                const int dst_offset =
                        oc_idx * img_stride + (oh_idx * ow + ow_idx) * oc_step;
                KerNeonDotXXs2Nchw44Int8<
                        bias_mode, Op, ow_step, filter_size, oc_step, ow_step, stride>::
                        impl(src + src_offset, filter + weight_offset, bias + oc_idx,
                             dst + dst_offset, ic, ih, iw, ld_dst_oc, op);
            }
            if (ow_remain > 0) {
