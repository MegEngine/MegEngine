...
#define cb(step)                                                 \
    case step:                                                   \
        kern_big_oc_remain = ker_neon_dirctconv_2x2s2_oc8_ow8<   \
                bias_mode, Op, step, filter_size, 2, DstType>;   \
        kern_small_oc_remain = ker_neon_dirctconv_2x2s2_oc4_ow8< \
...
#undef cb
    ...
    for (size_t oc_idx = 0; oc_idx < oc_end; oc_idx += big_oc_step) {
        ...
        for (size_t oh_idx = 0; oh_idx < oh; oh_idx += oh_step) {
            ...
            if (ow_remain > 0) {
                const size_t src_offset = (oh_idx * stride_h * iw + ow_end * stride_w) *
                                          ic_step * pack_iw_len;
                const size_t dst_offset =
...
#define cb(step)                                                     \
    case step:                                                       \
        kern_small_oc_remain = KerNeonDirectStride2Int8<             \
...
struct ConvDirectInt8Nchw44Choose<bias_mode, Op, filter_size, DstType, 2> {
    static void impl(
            ...
            DstType* dst, const size_t oc, const size_t ic, const size_t ih,
            const size_t iw, const size_t oh, const size_t ow, const Op& op) {
        conv_direct_stride2_int8_nchw44_kern<bias_mode, Op, filter_size, DstType>(
