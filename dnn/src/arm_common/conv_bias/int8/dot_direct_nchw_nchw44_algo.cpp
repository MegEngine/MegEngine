...
static inline size_t get_perthread_cache_bytes(
        const int ic, const int ih2, const int iw2, const int stride) {
...
static void get_rectified_size(
    ...
    int oh = param.osz[0];
    int block_oh =
            l2_block_helper(param.nr_threads, oh, ic * iw * sizeof(int8_t) * stride_h);
...
static WorkspaceBundle get_bundle(const ConvBiasImpl::NCBKernSizeParam& param) {
    ...
    return {nullptr,
            {src_size * param.nr_threads, weight_size, temp_size * param.nr_threads}};
};

void do_weight_trans(
        const WorkspaceBundle& bundle, const ConvBiasImpl::NCBKernParam& kern_param,
...
static void do_conv_kern(
    ...
    int oh_block = l2_block_helper(
            kern_param.nr_threads, oh, ic * iw * sizeof(int8_t) * stride_h);
    ...
    const int32_t* bptr = kern_param.bias<dt_int32>(batch_id, group_id) + bias_offset;
    ...
    dot_direct_nchw_nchw44::conv_direct_int8_nchw_nchw44_dot<
...
SmallVector<ConvBiasImpl::NCBKern> ConvBiasImpl::AlgoDotS8DirectNCHWNCHW44::
