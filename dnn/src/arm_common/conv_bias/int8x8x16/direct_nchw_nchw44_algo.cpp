...
static void get_rectified_size(
    ...
    int block_oh =
            l2_block_helper(param.nr_threads, oh, ic * iw2 * stride_h * iw_expand);
...
static WorkspaceBundle get_bundle(const ConvBiasImpl::NCBKernSizeParam& param) {
    ...
    size_t weight_size =
            group * round_up(oc, 8) * ic * fh * fw * sizeof(int8_t) * weight_expand;
    size_t bisa_size = 0;
    if (param.bias_mode == BiasMode::BROADCAST_CHANNEL_BIAS && oc % pack_oc != 0) {
        bisa_size = round_up(oc, 8) * sizeof(int16_t);
    }
    return {nullptr, {src_size * param.nr_threads, weight_size, bisa_size}};
};

static inline void copy_pad_src(
        int8_t* sptr_base, const int8_t* sptr_origin, int ph, int pw, int pad_right,
        int ih, int iw, int iw2, int pad_top, int pad_bottom, int ic, int ic_stride) {
    constexpr int iw_expand = 8;
...
static void do_conv_kern(
    ...
    int oh_block = l2_block_helper(
            kern_param.nr_threads, oh, ic * iw2 * stride_h * src_expand);
    ...
    const int8_t* origin_sptr = static_cast<const int8_t*>(kern_param.src<int8_t>(
                                        batch_id, group_id, 0, 1, 1)) +
                                src_offset;
    const size_t src_size = get_perthread_cache_bytes(ic, ih2, iw2);
    int8_t* sptr = reinterpret_cast<int8_t*>(
    ...
    auto packed_weight =
            ...
            (group_id * oc * ic * fh * fw + oc_idx * ic * fh * fw) * weight_expand;
    //! get param
    ...
    i8i8i16_direct_nchw_nchw44::conv_direct_i8i8i16_nchw_nchw44<
            ...
            sptr, packed_weight, bptr, nullptr, dst, oc_block, ic, ih_real, iw2, oh,
...
SmallVector<ConvBiasImpl::NCBKern> ConvBiasImpl::AlgoI8x8x16DirectNCHWNCHW44::
...
#undef DISPATCH_CONV_KERN
    ...
    auto do_conv = [bundle, do_conv_fun, ncb_range](
                           ...
                           const NCBKernIndex& ncb_index) mutable {
        bundle.set(kern_param.workspace_ptr);
        do_conv_fun(bundle, kern_param, ncb_index, ncb_index.ndrange_id, ncb_range);
    };
