...
#undef cb
    ...
    for (int ic_idx = 0; ic_idx < ic; ic_idx += loop_ic_step) {
        const int8_t* src_row0 =
                src_ptr + ic_idx * ic_stride + 0 * iw * ic_step * src_expand_size;
        const int8_t* src_row1 =
                src_ptr + ic_idx * ic_stride + 1 * iw * ic_step * src_expand_size;
...
#define CALC_ONE_RESULT(_src0, _src1, _src2, _w0, _w1, _w2, _c)               \
    do {                                                                      \
        ...
        _c = vadd_s16(_c, vadd_s16(vget_low_s16(tmp0), vget_high_s16(tmp0))); \
                ...
                CALC_ONE_RESULT(
                        src[0], src[1], src[2], weight[0], weight[1], weight[2],
                        c[0][0]);
                src[4] = vld1q_s8(src_ic_0_3 + 4 * 16);
                CALC_ONE_RESULT(
                        src[1], src[2], src[3], weight[0], weight[1], weight[2],
                        c[0][1]);
                src[0] = vld1q_s8(src_ic_0_3 + 5 * 16);
                CALC_ONE_RESULT(
                        src[2], src[3], src[4], weight[0], weight[1], weight[2],
                        c[0][2]);
                src[1] = vld1q_s8(src_ic_0_3 + 6 * 16);
                CALC_ONE_RESULT(
                        src[3], src[4], src[0], weight[0], weight[1], weight[2],
                        c[0][3]);
                src[2] = vld1q_s8(src_ic_0_3 + 7 * 16);
                CALC_ONE_RESULT(
                        src[4], src[0], src[1], weight[0], weight[1], weight[2],
                        c[0][4]);
...
#define cb(step)                                                                \
    case step:                                                                  \
        kern_big_oc_remain =                                                    \
                ker_neon_dirctconv_2x2s1_oc8_ow8<bias_mode, step, filter_size>; \
        kern_small_oc_remain =                                                  \
                ker_neon_dirctconv_2x2s1_oc4_ow8<bias_mode, step, filter_size>; \
        break;
...
#define cb(step)                                                              \
    case step:                                                                \
        kern_small_oc_remain =                                                \
                KerNeonDirectStride1Int8<bias_mode, step, filter_size>::impl; \
        break;
