...
template <
        typename src_ctype, typename dst_ctype = src_ctype,
...
template <>
struct FuseAddReluOpBase<dt_quint8, dt_quint8> : BinaryOpBase<dt_quint8, dt_quint8> {
    float szp;
    float32x4_t vszp;
...
template <>
struct FuseAddReluOp<dt_quint8, dt_quint8> : FuseAddReluOpBase<dt_quint8, dt_quint8>,
                                             FuseAddReluOpCommon<float> {
    using FuseAddReluOpBase::FuseAddReluOpBase;
    using FuseAddReluOpBase::operator();
    constexpr static size_t SIMD_WIDTH = 16;
...
template <>
struct FuseAddReluOpBase<dt_qint32, dt_quint8> : BinaryOpBase<dt_qint32, dt_quint8> {
    using BinaryOpBase::BinaryOpBase;
    void operator()(
...
struct FuseAddReluOp<dt_qint32, dt_qint8, enable_opt_or_fixup>
    ...
    void operator()(
            const int32x4x2_t& vsrc0, const int32x4x2_t& vsrc1, dt_qint8* dst) const {
...
struct FuseAddReluOp<dt_qint32, dt_qint8, enable_opt_or_fixup>
    ...
    FuseAddReluOp(DType src0_dtype, DType src1_dtype, DType dst_dtype)
            : FuseAddReluOpBase(src0_dtype, src1_dtype, dst_dtype), FixupBase(scale0) {}

    FuseAddReluOp(float src0_scale, float src1_scale, float dst_scale)
            : FuseAddReluOpBase(src0_scale, src1_scale, dst_scale), FixupBase(scale0) {}
    ...
    int8x8_t operator()(const int32x4x2_t& vsrc0, const int32x4x2_t& vsrc1) const {
        if (enable_opt_or_fixup) {
            auto vitem0 =
                    vqrdmulhq_s32(vaddq_s32(vsrc0.val[0], vsrc1.val[0]), vmultiplier);
            auto vitem1 =
                    vqrdmulhq_s32(vaddq_s32(vsrc0.val[1], vsrc1.val[1]), vmultiplier);
...
struct FuseAddReluOp<dt_qint32, dt_quint8, enable_opt_or_fixup>
    ...
    constexpr static size_t SIMD_WIDTH = 4;
    void operator()(
            const int32x4x2_t& vsrc0, const int32x4x2_t& vsrc1, dt_quint8* dst) const {
        vst1_u8(reinterpret_cast<uint8_t*>(dst), operator()(vsrc0, vsrc1));
    }

    uint8x8_t operator()(const int32x4x2_t& vsrc0, const int32x4x2_t& vsrc1) const {
        if (enable_opt_or_fixup) {
            auto vitem0 = vmulq_f32(
                    vcvtq_f32_s32(vaddq_s32(vsrc0.val[0], vsrc1.val[0])),
                    this->vscale0);
