...
struct ElemArithKernWrapper<
        ...
        typename std::enable_if<
                ...
                !std::is_same<typename KernImpl::ctype, dt_bool>::value>::type> {
    typedef typename KernImpl::ctype ctype;
    ctype* dst;

#if MEGDNN_CC_CUDA
    __device__ void operator()(uint32_t idx, ctype x) { dst[idx] = KernImpl::apply(x); }
#endif
};
template <class KernImpl>
struct ElemArithKernWrapper<
...
struct ElemArithKernWrapper<
    ...
    ctype* dst;
#if MEGDNN_CC_CUDA
    __device__ __forceinline__ void operator()(uint32_t idx, ctype x, ctype y) {
        dst[idx] = KernImpl::apply(x, y);
    }
    __device__ __forceinline__ void operator()(uint32_t idx, vect_type x, vect_type y) {
...
struct ElemArithKernWrapper<
    ...
    __device__ __forceinline__ void operator()(
            uint32_t idx, ctype x, ctype y, ctype z) {
        dst[idx] = KernImpl::apply(x, y, z);
    }
    __device__ __forceinline__ void operator()(
            uint32_t idx, vect_type x, vect_type y, vect_type z) {
