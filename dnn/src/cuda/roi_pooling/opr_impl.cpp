        ...
        switch (param().mode) {                                                  \
            case param::ROIPooling::Mode::MAX:                                   \
                forward_proxy<T, MaxPooler<T>>(                                  \
                        nthreads, src.ptr<T>(), spatial_scale, channels, height, \
                        width, pooled_height, pooled_width, rois.ptr<T>(),       \
...
#define cb(DType)                                                                 \
    if (src.layout.dtype == DType()) {                                            \
        ...
        switch (param().mode) {                                                   \
            case param::ROIPooling::Mode::MAX:                                    \
                roi_pooling::backward_proxy<T, BwdMaxPooler<T>>(                  \
                        nthreads, diff.ptr<T>(), index.ptr<dt_int32>(), num_rois, \
                        spatial_scale, channels, height, width, pooled_height,    \
                        pooled_width, grad.ptr<T>(), rois.ptr<T>(), stream);      \
                break;                                                            \
            case param::ROIPooling::Mode::AVERAGE:                                \
                roi_pooling::backward_proxy<T, BwdAveragePooler<T>>(              \
                        nthreads, diff.ptr<T>(), index.ptr<dt_int32>(), num_rois, \
                        spatial_scale, channels, height, width, pooled_height,    \
                        pooled_width, grad.ptr<T>(), rois.ptr<T>(), stream);      \
                break;                                                            \
