...
__global__ void conv_kernel(
        ...
        const uint32_t OUT_BS, const uint32_t IC, const uint32_t ID, const uint32_t IH,
        const uint32_t IW, const uint32_t OC, const uint32_t OD, const uint32_t OH,
        const uint32_t OW, const uint32_t FD, const uint32_t FH, const uint32_t FW,
        const uint32_t SD, const uint32_t SH, const uint32_t SW, const uint32_t PD,
        const uint32_t PH, const uint32_t PW, const uint32_t DD, const uint32_t DH,
        const uint32_t DW) {
    ...
    for (; i < widthA; i += BM, offsetA += BM) {
        ...
        if (tidy < BM) {
            uint32_t fd2d = fd2 * DD, fh2d = fh2 * DH, fw2d = fw2 * DW;
            uint32_t tmp = offsetB + ic * ID * IH * IW + fd2d * IH * IW + fh2d * IW +
                           fw2d,
    ...
    do {                                                                              \
        if (src_tex.init_succ) {                                                      \
            ...
            kptr<<<blocks, threads, 0, stream>>>(                                     \
                    src_tex.val, filter_tex.val, dst, INP_BS, OUT_BS, IC, ID, IH, IW, \
                    OC, OD, OH, OW, FD, FH, FW, SD, SH, SW, PD, PH, PW, DD, DH, DW);  \
        } else {                                                                      \
            ...
            kptr<<<blocks, threads, 0, stream>>>(                                     \
                    src_buf, filter_buf, dst, INP_BS, OUT_BS, IC, ID, IH, IW, OC, OD, \
                    OH, OW, FD, FH, FW, SD, SH, SW, PD, PH, PW, DD, DH, DW);          \
        }                                                                             \
    } while (0)
