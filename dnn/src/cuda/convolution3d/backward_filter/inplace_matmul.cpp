...
void Convolution3DBackwardFilterImpl::AlgoInplaceMatmul::exec(
    ...
    size_t N = args.src_layout->shape[0], IC = fm.icpg, ID = args.src_layout->shape[2],
           ...
           OD = args.diff_layout->shape[2], OH = args.diff_layout->shape[3],
           OW = args.diff_layout->shape[4], FD = fm.spatial[0], FH = fm.spatial[1],
    ...
    convolution3d::exec_inplace_matmul_bwd_filter(
            args.diff_tensor->ptr<dt_float32>(), args.src_tensor->ptr<dt_float32>(),
            args.grad_tensor->ptr<dt_float32>(), N, args.src_layout->stride[0],
            args.diff_layout->stride[0], IC, ID, IH, IW, OC, OD, OH, OW, FD, FH, FW,
