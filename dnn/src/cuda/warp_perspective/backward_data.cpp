...
WorkspaceBundle WarpPerspectiveBackwardDataImpl::get_workspace_bundle(
    ...
    get_workspace(fdiff);
    get_workspace(fgrad);
    sizes.push_back(get_float32_workspace_in_bytes(fmat, mat_idx, fdiff, fgrad));
    return {ptr, std::move(sizes)};
}

void WarpPerspectiveBackwardDataImpl::exec(
        _megdnn_tensor_in smat, _megdnn_tensor_in mat_idx, _megdnn_tensor_in sdiff,
        _megdnn_tensor_out sgrad, _megdnn_workspace sworkspace) {
    check_exec(
            smat.layout, mat_idx.layout, sdiff.layout, sgrad.layout, sworkspace.size);
    TensorND mat = smat;
    TensorND diff = sdiff;
    TensorND grad = sgrad;
    auto bundle = get_workspace_bundle(
            sworkspace.raw_ptr, smat.layout, mat_idx.layout, sdiff.layout,
            sgrad.layout);
    ...
    }
    {
        ...
        if (batch_x_channel_size <= max_batch_x_channel) {
            warp_perspective::backward_data_proxy(
                    ...
                    grad.ptr<dt_float32>(), reinterpret_cast<float*>(workspace.raw_ptr),
                    N, grad.layout.shape[0], C, IH, IW, OH, OW, bval, bmode, stream);
        } else {
            ...
            while (N > 0) {
                ...
                warp_perspective::backward_data_proxy(
                        ...
                        reinterpret_cast<float*>(workspace.raw_ptr), curr_batch_size,
                        grad.layout.shape[0], C, IH, IW, OH, OW, bval, bmode, stream);
...
size_t WarpPerspectiveBackwardDataImpl::get_float32_workspace_in_bytes(
        ...
        const TensorLayout& diff, const TensorLayout& grad) const {
    auto N = grad.shape[0], C = grad.shape[1], IH = grad.shape[2], IW = grad.shape[3];
