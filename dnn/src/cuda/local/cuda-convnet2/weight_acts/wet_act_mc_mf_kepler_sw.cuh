...
#pragma unroll
    ...
    /*
     * Note; iterating this way is about 1% slower and uses a few more registers than
     * iterating over the modules linearly. But it's consistent with the preload
     * routines, so I'm using it.
     */
            ...
            for (int f = 0; f < filtersPerThread; f++) {
                if (blockFilterIdx + threadIdx.x + f * B_X < numFilters) {
                    targets[c * B_Y * filterPixels * numFilters + f * B_X] =
                            scaleTargets * targets[c * B_Y * filterPixels * numFilters +
                                                   f * B_X] +
...
#define WET_ACT_MC_MF_KEPLER_SW_HEAD \
    template __global__ void conv_weight_acts_mc_mf_kepler_sw
