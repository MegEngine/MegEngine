...
filterActs_YxX_color_preload_ty_4_tx_32_f_16_cc_3_setImgCoords(
        int fPidx, int imgLoadModPosY, int imgLoadModPosX, int imgSizeX, int filterSize,
        int& iPidx) {
...
#define FA_COLOR3_IMPRELOAD_TX(c, i)                                             \
    imPreload[c][i] =                                                            \
            iPidxNext < 0 || (checkImgBounds && myImgIdx + i * B_X >= numImages) \
                    ...
                    : tex1Dfetch<float>(                                         \
                              images,                                            \
                              imagesOffset2 + c * imgPixels * imgStride + i * B_X);
...
__global__ void filterActs_YxX_color_preload_ty_4_tx_32_i_4_f_16_px_4_cc_3_tex(
    ...
    const int filtersOffset =
            blockFilterIdx + shFilterLoadY * numFilters + shFilterLoadX +
        ...
        for (int i = 0; i < imgsPerThread; i++) {
            if (iPidxNext >= 0 && (!checkImgBounds || myImgIdx + i * B_X < numImages)) {
                imPreload[c][i] = tex1Dfetch<float>(
                        images, imagesOffset + (c * imgPixels + iPidxNext) * imgStride +
                                        i * B_X);
            } else {
                imPreload[c][i] = 0;
            }
...
#pragma unroll
        ...
        filterActs_YxX_color_preload_ty_4_tx_32_f_16_cc_3_setImgCoords(
                fPidxNext + ty, imgLoadModPosY, imgLoadModPosX, imgSizeX, filterSize,
                iPidxNext);
                ...
                fPreload[c][pp] =
                        fPidxNext + pp * (B_X / filtersPerThread) + shFilterLoadY >=
                                ...
                                : tex1Dfetch<float>(
                                          filters,
                                          filtersOffset2 +
                                                  c * numFilters * filterPixels +
                                                  pp * (B_X / filtersPerThread) *
                                                          numFilters);
            }
        }
#pragma unroll
            ...
            for (int p = 0; p < pixelCache; p += 2) {
                if (p + shFilterLoadY < filterPixels) {
                    fPreload[c][p / 2] = tex1Dfetch<float>(
                            filters, filtersOffset + p * numFilters +
                                             c * numFilters * filterPixels);
                } else {
                    fPreload[c][p / 2] = 0;
                }
 ...
 * targets:     (numFilters, numModulesY, numModulesX, numImages)
 *
 * Note: in git there's a 1.5% faster version of this which sues 167 registers instead
 * of 154... it's basically the same thing, but it doesn't do the next-pixel
 * computation. It just avoids pre-loading when it rolls over to the next pixel.
 */
template <
...
__global__ void filterActs_YxX_sparse2_preload_ty_4_tx_32_i_4_f_16_c_4(
        ...
        const float scaleOutputs, const bool conv /*, const bool noloads*/) {
    __shared__ float shFilters
            [colorCache]
            [B_Y *
             filtersPerThread];  // pre-load 1 pixel from B_Y*filtersPerThread filters
    ...
    for (int imgY = imgStartY; imgY < imgEndY; ++imgY) {
        ...
        for (int imgX = imgStartX; imgX < imgEndX; ++imgX) {
            ...
            for (int oc = 0; oc < numFilterColors;
                ...
                const float* ff = &filters
                                          [numFilters *
                                           ((oc + colorCache) * filterPixels + fPidx)];
                const float* mm =
...
void _filterActs(
    ...
    // printf("rows: %d, pixels: %d, colors: %d\n", images.getNumRows(), imgPixels,
    // numImgColors); images.printShape("images");
    ...
    int imgStride =
            images.getStride();  // images does not need to be a contiguous matrix
    ...
    if (numImgColors <= 3) {
        // Special kernels written for colors = 3, filters = 64 and colors = 3, filters
        // = 48 cases. The remaining cases use the old routines.
        // TODO: Modernize the remaining cases if you care about them.
        filtersPerThread = numFiltersPerGroup % 64 == 0 ? 16
                         : numFiltersPerGroup % 48 == 0 ? 12
                         : numFiltersPerGroup % 32 == 0 ? 8
                                                        : 4;
    } else {
        filtersPerThread = numFiltersPerGroup % 64 == 0 ? 16
                         : numFiltersPerGroup % 32 == 0 ? 8
    ...
    dim3 blocks =
            dim3(DIVUP(numImages, threads.x * imgsPerThread),
                 numModules * DIVUP(numFilters, (threads.y * filtersPerThread)));
    ...
    // imgsPerThread = 1. In principle it doesn't have to be this way, and you may want
        ...
        if (checkImgBounds == false) {
            if (numFilterColors % 8 == 0) {
                if (numImages % 128 == 0) {
                    if (numFiltersPerGroup % 128 == 0) {
                        if (previous_limit) {
                            if (images.getNumDataBytes() < TEXTURE_SIZE_MAX) {
                                ...
                                filterActs_YxX_sparse2_preload_ty_4_tx_32_i_4_f_16_c_4_tex<
                                        ...
                                        <<<blocks, threads, 0, stream>>>(
                                                ...
                                                targets.getDevData(), numImages,
                                                numFilters, imgSizeY, imgSizeX,
                            ...
                            filterActs_YxX_sparse2<4, 32, 4, 16, 4, false, false>
                                    <<<blocks, threads, 0, stream>>>(
                                            images.getDevData(), filters.getDevData(),
                                            targets.getDevData(), numImages, numFilters,
                                            imgSizeY, imgSizeX, filterSize,
                                            paddingStart, moduleStride, numModulesY,
                                            numModulesX, imgStride, numImgColors,
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 4, 8, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 4, 8, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                                        ...
                                        moduleStride, numModulesY, numModulesX,
                                        imgStride, numImgColors, numGroups,
                    ...
                    if (numFiltersPerGroup % 128 == 0) {
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<8, 32, 2, 16, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<8, 32, 2, 16, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 2, 16, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 2, 16, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 2, 8, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 2, 8, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                    ...
                    if (numFiltersPerGroup % 128 == 0) {
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<8, 32, 1, 16, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<8, 32, 1, 16, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 1, 16, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 1, 16, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 1, 8, 8, false, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 1, 8, 8, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                    ...
                    if (numFiltersPerGroup % 64 == 0) {
                        if (previous_limit) {
                            ...
                            filterActs_YxX_color_preload_ty_4_tx_32_i_4_f_16_px_4_cc_3_tex<
                                    ...
                                    <<<blocks, threads, 0, stream>>>(
                                            ...
                                            imgSizeY, imgSizeX, filterSize,
                                            paddingStart, moduleStride, numModulesY,
                                            numModulesX, imgStride, scaleTargets,
                                            scaleOutput, conv);
                        } else {
                            cudaFuncSetCacheConfig(
                                    filterActs_YxX_color<
                                            4, 32, 4, 16, 3, 4, false, false>,
                                    cudaFuncCachePreferShared);
                            filterActs_YxX_color<4, 32, 4, 16, 3, 4, false, false>
                                    <<<blocks, threads, 0, stream>>>(
                                            images.getDevData(), filters.getDevData(),
                                            targets.getDevData(), numImages, numFilters,
                                            imgSizeY, imgSizeX, filterSize,
                        ...
                        if (previous_limit) {
                            ...
                            cudaFuncSetCacheConfig(
                                    filterActs_YxX_color<
                                            4, 32, 4, 12, 3, 4, false, false>,
                                    cudaFuncCachePreferShared);
                            filterActs_YxX_color<4, 32, 4, 12, 3, 4, false, false>
                                    <<<blocks, threads, 0, stream>>>(
                                            images.getDevData(), filters.getDevData(),
                                            targets.getDevData(), numImages, numFilters,
                                            imgSizeY, imgSizeX, filterSize,
                        ...
                        filterActs_YxX_color<4, 32, 4, 8, 3, 4, false, false>
                                <<<blocks, threads, 0, stream>>>(
                                        ...
                                        imgSizeY, imgSizeX, filterSize, paddingStart,
                                        moduleStride, numModulesY, numModulesX,
                                        imgStride, scaleTargets, scaleOutput, conv);
                    } else if (numFiltersPerGroup % 1 == 0) {
                        cudaFuncSetCacheConfig(
            ...
            if (numFilterColors % 8 == 0) {
                if (numImages % 128 == 0) {
                    if (numFiltersPerGroup % 128 == 0) {
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 4, 8, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 4, 8, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                    ...
                    if (numFiltersPerGroup % 128 == 0) {
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<8, 32, 2, 16, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<8, 32, 2, 16, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 2, 16, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 2, 16, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 2, 8, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 2, 8, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                    ...
                    if (numFiltersPerGroup % 128 == 0) {
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<8, 32, 1, 16, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<8, 32, 1, 16, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 1, 16, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 1, 16, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        cudaFuncSetCacheConfig(
                                filterActs_YxX_sparse2<4, 32, 1, 8, 8, true, false>,
                                cudaFuncCachePreferShared);
                        filterActs_YxX_sparse2<4, 32, 1, 8, 8, true, false>
                                <<<blocks, threads, 0, stream>>>(
                                        images.getDevData(), filters.getDevData(),
                        ...
                        if (previous_limit) {
                            ...
                            cudaFuncSetCacheConfig(
                                    filterActs_YxX_color<
                                            4, 32, 4, 16, 3, 4, true, false>,
                                    cudaFuncCachePreferShared);
                            filterActs_YxX_color<4, 32, 4, 16, 3, 4, true, false>
                                    <<<blocks, threads, 0, stream>>>(
                                            images.getDevData(), filters.getDevData(),
                                            targets.getDevData(), numImages, numFilters,
                                            imgSizeY, imgSizeX, filterSize,
                        ...
                        if (previous_limit) {
                            ...
                            cudaFuncSetCacheConfig(
                                    filterActs_YxX_color<
                                            4, 32, 4, 12, 3, 4, true, false>,
                                    cudaFuncCachePreferShared);
                            filterActs_YxX_color<4, 32, 4, 12, 3, 4, true, false>
                                    <<<blocks, threads, 0, stream>>>(
                                            images.getDevData(), filters.getDevData(),
                                            targets.getDevData(), numImages, numFilters,
                                            imgSizeY, imgSizeX, filterSize,
...
void convFilterActs(
    ...
    convFilterActs(
            stream, images, filters, targets, imgSizeY, numModulesY, numModulesX,
            paddingStart, moduleStride, numImgColors, numGroups, 0, 1);
}
