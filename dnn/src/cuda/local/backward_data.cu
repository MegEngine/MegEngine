...
bool can_backward_data_proxy_convnet(
...
size_t get_workspace_in_floats_backward_data_proxy_convnet(
...
void backward_data_proxy_convnet(
        const float* filter, const float* diff, float* grad, float* workspace, size_t N,
        size_t IC, size_t IH, size_t IW, size_t OC, size_t OH, size_t OW, size_t FH,
        size_t FW, size_t INs, size_t ONs, size_t PH, size_t /* PW */, size_t SH,
        size_t /* SW */, cublasHandle_t cublas_handle, cudaStream_t stream, float* one,
        float* zero) {
    MemorySegment mhid_n(const_cast<float*>(diff)), mfilter(const_cast<float*>(filter)),
            mtarget_n(grad), mtarget_t(workspace), mhid_t(workspace + N * IC * IH * IW);
    NVMatrix nvhid_n(&mhid_n, N, OC * OH * OW, ONs),
            nvfilter(&mfilter, OH * OW * IC * FH * FW, OC),
            nvtarget_n(&mtarget_n, N, IC * IH * IW, INs),
            nvhid_t(&mhid_t, OC * OH * OW, N), nvtarget_t(&mtarget_t, IC * IH * IW, N);
    nvhid_n.transpose(nvhid_t, cublas_handle, one, zero);

    localImgActs(
            stream, nvhid_t, nvfilter, nvtarget_t, IH, IW, OH, -static_cast<int>(PH),
            SH, IC, 1);
    after_kernel_launch();
