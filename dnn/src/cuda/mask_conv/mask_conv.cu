...
__global__ void set_zero_by_mask_kernel(
        float* dst, const ctype* mask, size_t N, size_t mask_size) {
...
template <typename ctype>
__global__ void mask_propagate_kernel(
    ...
    for (int fh = 0; fh < FH; ++fh) {
        for (int fw = 0; fw < FW; ++fw) {
            ...
            if (ih < 0 || ih >= IH || iw < 0 || iw >= IW || src[ih * IW + iw] == 0) {
                continue;
            }
            dst[dst_idx] = 1;
            return;
...
void mask_propagate_exec_proxy(
    ...
    mask_propagate_kernel<ctype><<<DIVUP(OH * OW, NR_THREADS), NR_THREADS, 0, stream>>>(
...
#define INST(ctype)                                                                   \
    template void mask_propagate_exec_proxy<ctype>(                                   \
            const ctype* src, ctype* dst, size_t IH, size_t IW, size_t OH, size_t OW, \
            size_t FH, size_t FW, size_t SH, size_t SW, size_t PH, size_t PW,         \
