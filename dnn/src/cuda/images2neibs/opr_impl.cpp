...
void Images2NeibsForwardImpl::exec(
    ...
    int N = src.layout[0], C = src.layout[1], IH = src.layout[2], IW = src.layout[3];
...
void Images2NeibsBackwardImpl::exec(
    ...
    auto stream = cuda_stream(handle());
    int N = grad.layout[0], C = grad.layout[1], IH = grad.layout[2],
        IW = grad.layout[3];
