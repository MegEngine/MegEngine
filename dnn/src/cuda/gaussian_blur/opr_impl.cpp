...
void gaussian_blur_exec(
        ...
        uint8_t* kernel_ptr, size_t kernel_height, size_t kernel_width, double sigma_x,
        double sigma_y, param::GaussianBlur::BorderMode bmode, cudaStream_t stream) {
    megdnn_assert(IC == 1_z || IC == 3_z);
#define INIT_KERN(bmode)                                                             \
    if (IC == 1) {                                                                   \
        gaussian_blur::gaussian_blur<ctype, 1, bmode>(                               \
                src, dst, N, IH, IW, stride0, stride1, stride2, stride3, kernel_ptr, \
                kernel_height, kernel_width, sigma_x, sigma_y, stream);              \
    } else {                                                                         \
...
void GaussianBlurImpl::exec(
    ...
    megdnn_assert(
            src.layout.dtype == dtype::Uint8() || src.layout.dtype == dtype::Float32());
...
#define cb(DType)                                                                     \
    if (src.layout.dtype == DType()) {                                                \
        ...
        intl::gaussian_blur_exec<ctype>(                                              \
                ...
                src.layout.stride[1], src.layout.stride[2], src.layout.stride[3],     \
                workspace.ptr<uint8_t>(), m_kernel_height, m_kernel_width, m_sigma_x, \
                m_sigma_y, param().border_mode, stream);                              \
    }
