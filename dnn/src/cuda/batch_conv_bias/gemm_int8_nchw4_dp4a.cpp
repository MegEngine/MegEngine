...
void dispatch_kernel(
    ...
    if (img_pixels % 4 == 0) {
        kern_wrapper =
                do_batch_conv_bias_int8_gemm_ncdiv4hw4_ldg_128<BiasVisitor, Epilogue>;
    } else {
        kern_wrapper = do_batch_conv_bias_int8_gemm_ncdiv4hw4<BiasVisitor, Epilogue>;
    }
...
bool BatchConvBiasForwardImpl::AlgoInt8NCHW4DotProdGemm::is_available(
    ...
    available &= (fh == 1 && sh == 1 && fw == 1 && sw == 1 && ph == 0 && pw == 0);
...
size_t BatchConvBiasForwardImpl::AlgoInt8NCHW4DotProdGemm::get_workspace_in_bytes(
        const SizeArgs& /* args */) const {
    return 0;
}
