...
bool LocalShareBackwardFilterImpl::AlgoBatchedMatMul::is_available(
    ...
    available &=
            (src_dtype == diff_dtype && src_dtype == grad_dtype &&
...
WorkspaceBundle LocalShareBackwardFilterImpl::AlgoBatchedMatMul::get_workspace_bundle(
        dt_byte* raw_ptr, const SizeArgs& args) const {
    auto&& param = args.opr->param();
    unpack_local_share_params(
            args.src_layout, args.grad_layout, args.diff_layout, param);
    using Param = LocalShare::Param;
...
void LocalShareBackwardFilterImpl::AlgoBatchedMatMul::exec(const ExecArgs& args) const {
    auto&& param = args.opr->param();
    unpack_local_share_params(
            args.src_layout, args.grad_layout, args.diff_layout, param);
    using Param = LocalShare::Param;
    ...
    {
        TensorLayout B1{
                {groups, sgh, sgw, ocpg, n, ho / sgh, wo / sgw}, dtype::Float32()};
