...
void TopKImpl::dispatch_with_ctype(
    ...
    switch (param().mode) {
        case Param::Mode::KTH_ONLY:
            cuda_check(topk::find_kth_radix<ctype>(
                    data, values, workspace, m, n, lda, k, grid_dim_y_limit, stream));
            return;
        case Param::Mode::VALUE_IDX_NOSORT: {
            ...
            cuda_check(topk::find_kth_radix<ctype>(
                    data, thresh, real_wk, m, n, lda, k, grid_dim_y_limit, stream));
            cuda_check(topk::topk_select<ctype>(
                    data, thresh, values, indices, real_wk, m, n, lda, k,
        ...
        case Param::Mode::VALUE_IDX_SORTED: {
            ...
            cuda_check(topk::find_kth_radix<ctype>(
                    data, thresh, real_wk, m, n, lda, k, grid_dim_y_limit, stream));
            cuda_check(topk::topk_select<ctype>(
                    data, thresh, nosort_values, nosort_idx, real_wk, m, n, lda, k,
                    grid_dim_y_limit, stream));
            argsort::forward(
                    nosort_values, values, indices, real_wk, m, std::abs(k), k > 0,
                    stream, nosort_idx);
            return;
        }
    }
...
void TopKImpl::do_exec(
    ...
    switch (data.layout.dtype.enumv()) {
        case DTypeEnum::Float32:
            dispatch_with_ctype<float>(
                    k, data.layout[0], data.layout[1], data.layout.stride[0],
                    data.ptr<float>(), values.ptr<float>(), indices, workspace.raw_ptr);
            return;
        case DTypeEnum::Int32:
            dispatch_with_ctype<int32_t>(
                    k, data.layout[0], data.layout[1], data.layout.stride[0],
                    data.ptr<int32_t>(), values.ptr<int32_t>(), indices,
                    workspace.raw_ptr);
            return;
#if !MEGDNN_DISABLE_FLOAT16
        case DTypeEnum::Float16:
            dispatch_with_ctype<dt_float16>(
                    k, data.layout[0], data.layout[1], data.layout.stride[0],
                    data.ptr<dt_float16>(), values.ptr<dt_float16>(), indices,
                    workspace.raw_ptr);
            return;
#endif
        default:
...
size_t TopKImpl::get_workspace_in_bytes(
    ...
    size_t grid_dim_y_limit = concrete_handle(handle())->device_prop().maxGridSize[1];
    megdnn_assert(
