...
__global__ void cuda_last_contiguous_kern(
    ...
    remain_size = remain_size > contiguous_size ? contiguous_size : remain_size;
...
__global__ void cuda_last_contiguous_large_kern(
    ...
    uint32_t idx =
            contig_idx * contiguous_size + contig_block_idx * contig_size_each_block;
    if (idx >= size)
        return;

    uint32_t remain = contiguous_size - contig_block_idx * contig_size_each_block;
    if (remain > contig_size_each_block)
        remain = contig_size_each_block;
 ...
 */
template <typename OpCaller>
__global__ void cuda_last_contiguous_pack_kern(
        OpCaller op_caller, uint32_t contiguous_size, uint32_t size) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    idx *= contiguous_size;
    if (idx >= size)
    ...
    void do_run() {
        ...
        if (m_contiguous_size >= 32) {
            ...
            if (m_contiguous_size > MAX_CONTIG_SIZE_EACH_BLOCK) {
                ...
                contig_block_size = (m_contiguous_size + contig_size_each_block - 1) /
            ...
            (*fptr)<<<grid_size, block_size, 0, m_stream>>>(
                    caller, m_contiguous_size, size, contig_size_each_block,
                    contig_block_size);
        } else {
            ...
            get_last_contiguous_launch_spec(
                    reinterpret_cast<const void*>(fptr), size, m_contiguous_size,
                    &grid_size, &block_size);
            Caller caller;
            caller.par0.host_init(m_param[0], grid_size, block_size);
            caller.par1.host_init(m_param[1], grid_size, block_size);
            (*fptr)<<<grid_size, block_size, 0, m_stream>>>(
                    caller, m_contiguous_size, size);
        }
        after_kernel_launch();
    }
...
public:
    UserOpInvoker(
            const ElemwiseOpParamN<2>& param, cudaStream_t stream,
            const size_t contiguous_size)
            : m_param(param), m_stream(stream), m_contiguous_size(contiguous_size) {
        m_invoked = false;
        dispatch0();
        megdnn_assert(m_invoked);
    }
