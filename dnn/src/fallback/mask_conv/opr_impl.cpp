...
void img2col_mask(
        const float* src, float* dst, const size_t OC, const size_t OH, const size_t OW,
        const size_t IC, const size_t IH, const size_t IW, const size_t FH,
        const size_t FW, const size_t SH, const size_t SW, const size_t DH,
        const size_t DW, const unsigned int* maskInd, const size_t maskN) {
...
void get_mask_index(
        const ctype* mask, const size_t OH, const size_t OW, unsigned int* maskInd,
        size_t* maskN) {
    size_t length = 0;
    rep(oh, OH) rep(ow, OW) {
        size_t idx = oh * OW + ow;
...
void index_to_dst(
        const float* result, float* dst, const size_t OC, const size_t OH,
        const size_t OW, unsigned int* maskInd, const size_t maskN) {
...
template <typename ctype>
void exec_internel(
        const float* src, const float* filter, const ctype* mask, float* dst,
        WorkspaceBundle wbundle, MatrixMul* opr, size_t N, size_t IC, size_t OC,
        size_t IH, size_t IW, size_t OH, size_t OW, size_t PH, size_t PW, size_t SH,
        size_t SW, size_t FH, size_t FW, size_t DH, size_t DW, bool is_xcorr) {
    ...
    rep(n, N) {
        ...
        if (is_xcorr) {
            img2col_mask<true>(
                    src_t, B_mat, OC, OH, OW, IC, IH2, IW2, FH, FW, SH, SW, DH, DW,
                    maskInd, maskN);
        } else {
            img2col_mask<false>(
                    src_t, B_mat, OC, OH, OW, IC, IH2, IW2, FH, FW, SH, SW, DH, DW,
        ...
        TensorND A((float*)filter, TensorLayout({OC, IC * FH * FW}, dtype::Float32())),
                B((float*)B_mat, TensorLayout({IC * FH * FW, maskN}, dtype::Float32())),
                C((float*)result, TensorLayout({OC, maskN}, dtype::Float32()));

        Workspace workspace(
                static_cast<megdnn::dt_byte*>(matmul_workspace_ptr), matmul_wsize);

        opr->exec(A, B, C, workspace);

        index_to_dst(result, dst + n * OC * OH * OW, OC, OH, OW, maskInd, maskN);
    }
}
...
void MaskConvForwardImpl::exec(
    ...
    check_exec(src.layout, filter.layout, mask.layout, dst.layout, workspace.size);
...
#define cb(DType)                                                                     \
    if (mask.layout.dtype == DType()) {                                               \
        ...
        MEGDNN_DISPATCH_CPU_KERN(                                                     \
                ...
                exec_internel<ctype>(                                                 \
                        src.ptr<float>(), filter.ptr<float>(), mask.ptr<ctype>(),     \
                        dst.ptr<float>(), wbundle, m_matmul_opr.get(), N, IC, OC, IH, \
                        IW, OH, OW, PH, PW, SH, SW, FH, FW, DH, DW, is_xcorr););      \
        return;                                                                       \
    }
...
size_t MaskConvForwardImpl::get_workspace_in_bytes(
    ...
    megdnn_ignore(mask);
    size_t OC = filter[0], IC = filter[1], IH = src[2], IW = src[3], OH = dst[2],
           OW = dst[3], FH = filter[2], FW = filter[3], PH = param().pad_h,
           PW = param().pad_w;
    auto wbundle = get_wbundle(OC, OH, OW, IC, IH, IW, FH, FW, PH, PW);
