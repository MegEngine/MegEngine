        ...
        for (size_t i = ndim; i > 0; --i) {
            ...
            if (times.shape[j] != 1) {
                ...
                auto m = std::accumulate(
                        src.shape, src.shape + i, 1_z, SafeMultiplies<size_t>());
                // n = dshape[i]*...
                auto n = std::accumulate(
                        dst.shape + i, dst.shape + ndim, 1_z, SafeMultiplies<size_t>());
