...
struct StrategyHashParamHash {
    uint64_t operator()(const StrategyHashParam& sparam) const {
        ...
        uint64_t result = static_cast<uint64_t>(sparam.param.src_type.enumv()) + base;
        result = result ^
                 ((static_cast<uint64_t>(sparam.param.dst_type.enumv()) + base) << 3);
        result =
                result ^
                ((static_cast<uint64_t>(sparam.param.filter_type.enumv()) + base) << 6);
        result = result ^
                 ((static_cast<uint64_t>(sparam.param.bias_type.enumv()) + base) << 9);
        result = result ^ ((static_cast<uint64_t>(sparam.format) + base) << 12);
        result = result ^ ((static_cast<uint64_t>(sparam.packmode) + base) << 15);
        result = result ^ ((static_cast<uint64_t>(sparam.block_m) + base) << 18);
        result = result ^ ((static_cast<uint64_t>(sparam.block_n) + base) << 22);
        result = result ^ ((static_cast<uint64_t>(sparam.block_k) + base) << 26);
        result = result ^ ((static_cast<uint64_t>(sparam.kernel) + base) << 30);
        result = result ^ ((static_cast<uint64_t>(sparam.stride) + base) << 34);
        result = result ^ ((static_cast<uint64_t>(sparam.is_square) + base) << 35);
        result = result ^ ((static_cast<uint64_t>(sparam.is_xcorr) + base) << 36);
        return result;
    };
};
...
struct StrategyHashParamEqual {
    bool operator()(
            const StrategyHashParam& param1, const StrategyHashParam& param2) const {
        bool flags = true;
...
class StrategyDelegationStorage {
    DNN_MUTEX m_mtx;
    std::unordered_map<
            StrategyHashParam, std::unique_ptr<StrategyBase>, StrategyHashParamHash,
...
public:
    ...
    Strategy* get(
            ...
            const fallback::ConvBiasImpl::NCBKernSizeParam& param, StrategyType stype);
};
...
#define cb2(                                                                         \
        _i_src_type, _i_bias_type, _i_dst_type, _src_ctype, _bias_ctype, _dst_ctype, \
        _strategytype)                                                               \
...
#if MEGDNN_AARCH64 || MEGDNN_ARMV7
        cb2(dtype::Quantized8Asymm, dtype::QuantizedS32, dtype::QuantizedS32, dt_uint8,
            dt_int32, dt_int32, StrategyType::QUINT8x8x32);

        cb2(dtype::Quantized8Asymm, dtype::QuantizedS32, dtype::Quantized8Asymm,
            dt_uint8, dt_int32, dt_uint8, StrategyType::QUINT8x8x32x8);
#endif
        cb2(dtype::QuantizedS8, dtype::QuantizedS32, dtype::QuantizedS32, dt_int8,
            dt_int32, dt_int32, StrategyType::QINT8x8x32);

        cb2(dtype::QuantizedS8, dtype::QuantizedS32, dtype::QuantizedS8, dt_int8,
            dt_int32, dt_int8, StrategyType::QINT8x8x32x8);
#undef cb1
#undef cb2
        megdnn_throw("not support datatype in im2col strategy\n");
    }

#define cb1(_format, _packmode, _dt, _post_ctype, _postprocess_mode, _midout_tag)   \
    MIDOUT_BEGIN(                                                                   \
            megdnn_fallback_im2col_factory_make_strategy, midout_iv(_midout_tag)) { \
        if (param.filter_type.enumv() == DTypeTrait<_dt>::enumv) {                  \
...
#define cb2(                                                                         \
        _format, _packmode, _i_src_type, _i_bias_type, _i_dst_type, _src_ctype,      \
    ...
    MIDOUT_BEGIN(                                                                    \
        ...
        if (param.filter_type.enumv() == param.src_type.enumv() &&                   \
            ...
            return std::make_unique<Strategy<                                        \
                    ...
                    _postprocess_mode, PackMode::_packmode, FormatMode::_format>>(); \
        }                                                                            \
    }                                                                                \
    MIDOUT_END();                                                                    \
    return {};
#define cb3(                                                                         \
    ...
    static std::unique_ptr<StrategyBase> make_default_strategy(
        ...
        switch (strategytype) {
                ...
                if (format == param::ConvBias::Format::NCHW) {
                    ...
                    if ((matmul_block.m == 8 || matmul_block.m == 4) &&
                        ...
                        MIDOUT_BEGIN(
                            ...
                            return std::make_unique<StrategyFuseXx12x1Nchw44K3x3S2<
                                    float, float, PostprocessMode::FLOAT>>();
                        }
                        MIDOUT_END();
                        return {};
                    }
            ...
            case StrategyType::FLOAT16_FLOAT16:
                cb1(NCHW, DEFAULT, dt_float16, dt_float16, PostprocessMode::NO_PROCESS,
                    "DefaultStrategyType::FLOAT16_FLOAT16"_hash);
                break;
#endif
            case StrategyType::INT8x8x32:
                if (format == param::ConvBias::Format::NCHW) {
                    cb3(NCHW, DEFAULT, dt_int8, dt_int32, dt_int32, dt_int8, dt_int32,
                    ...
                    cb3(NCHW44, DEFAULT, dt_int8, dt_int32, dt_int32, dt_int8, dt_int32,
                        dt_int32, PostprocessMode::ADD_BIAS,
                        "DefaultStrategyType::INT8x8x32"_hash);
                } else {
                    megdnn_throw(ssprintf(
            ...
            case StrategyType::INT8x8x16:
                if (format == param::ConvBias::Format::NCHW) {
                    cb3(NCHW, DEFAULT, dt_int8, dt_int16, dt_int16, dt_int8, dt_int16,
                        dt_int16, PostprocessMode::ADD_BIAS,
                        "DefaultStrategyType::INT8x8x16"_hash);
                } else if (format == param::ConvBias::Format::NCHW44) {
                    cb3(NCHW44, DEFAULT, dt_int8, dt_int16, dt_int16, dt_int8, dt_int16,
                        dt_int16, PostprocessMode::ADD_BIAS,
                        "DefaultStrategyType::INT8x8x16"_hash);
                } else {
                    megdnn_throw(ssprintf(
            ...
            case StrategyType::QUINT8x8x32:
                cb3(NCHW, DEFAULT, dtype::Quantized8Asymm, dtype::QuantizedS32,
                    ...
                    PostprocessMode::ADD_BIAS, "DefaultStrategyType::QUINT8x8x32"_hash);
                break;
            ...
            case StrategyType::QINT8x8x32:
                if (format == param::ConvBias::Format::NCHW) {
                    cb3(NCHW, DEFAULT, dtype::QuantizedS8, dtype::QuantizedS32,
                        ...
                        format == param::ConvBias::Format::NCHW44 ||
                        format == param::ConvBias::Format::NCHW44_DOT) {
                    cb3(NCHW44, DEFAULT, dtype::QuantizedS8, dtype::QuantizedS32,
                        dtype::QuantizedS32, dt_int8, dt_int32, dt_int32,
                        PostprocessMode::ADD_BIAS,
                ...
                if (format == param::ConvBias::Format::NCHW) {
                    ...
                    if (format == param::ConvBias::Format::NCHW44) {
                        ...
                        if (matmul_block.m == 4 && matmul_block.n == 4 &&
                            matmul_block.k == 16 && param.filter_meta.spatial[0] == 3 &&
                            param.filter_meta.spatial[1] == 3 &&
                            ...
                            MIDOUT_BEGIN(
                                ...
                                return std::make_unique<StrategyFuse4x4x16Nchw44<
                                        dt_qint32, dt_qint8,
                                        PostprocessMode::QUANTIZED>>();
                            }
                            MIDOUT_END();
                        ...
                        if (matmul_block.m == 8 && matmul_block.n == 12 &&
                            matmul_block.k == 4 && param.filter_meta.spatial[0] == 3 &&
                            param.filter_meta.spatial[1] == 3 &&
                            param.filter_meta.stride[0] == 1 &&
                            param.filter_meta.stride[1] == 1 &&
                            !param.filter_meta.should_flip) {
                            MIDOUT_BEGIN(
                                ...
                                return std::make_unique<StrategyFuse8x12x4Nchw44Dot<
                                        dt_qint32, dt_qint8,
                                        PostprocessMode::QUANTIZED>>();
                            }
                            MIDOUT_END();
                    ...
                    cb2(NCHW44, DEFAULT, dtype::QuantizedS8, dtype::QuantizedS32,
                        dtype::QuantizedS8, dt_int8, dt_int32, dt_int8,
                        PostprocessMode::QUANTIZED,
        ...
        megdnn_throw(ssprintf(
                "Unsupported strategy type %u in default mode",
    ...
    static std::unique_ptr<StrategyBase> make_nopack_strategy(
        ...
        switch (strategytype) {
            case StrategyType::FLOAT:
                cb1(NCHW, NO_PACK, dt_float32, dt_float32, PostprocessMode::FLOAT,
                    "NoPackStrategyType::FLOAT"_hash);
                break;
#if !MEGDNN_DISABLE_FLOAT16
            case StrategyType::FLOAT16_FLOAT16:
                cb1(NCHW, NO_PACK, dt_float16, dt_float16, PostprocessMode::NO_PROCESS,
                    "NoPackStrategyType::FLOAT16_FLOAT16"_hash);
                break;
#endif
#if __ARM_FEATURE_FP16_VECTOR_ARITHMETIC
            case StrategyType::FLOAT_FP16:
                cb1(NCHW, NO_PACK, dt_float16, __fp16, PostprocessMode::NO_PROCESS,
                    "NoPackStrategyType::FLOAT_FP16"_hash);
                break;
#endif
            case StrategyType::INT8x8x16:
                cb3(NCHW, NO_PACK, dt_int8, dt_int16, dt_int16, dt_int8, dt_int16,
                    dt_int16, PostprocessMode::ADD_BIAS,
                    "NoPackStrategyType::INT8x8x16"_hash);
                break;
            case StrategyType::INT8x8x32:
                cb3(NCHW, NO_PACK, dt_int8, dt_int32, dt_int32, dt_int8, dt_int32,
                    dt_int32, PostprocessMode::ADD_BIAS,
                    "NoPackStrategyType::INT8x8x32"_hash);
                break;
            default:
                megdnn_throw(ssprintf(
                        "Unsupported strategy type %u in no_pack mode",
                        uint32_t(strategytype)));
                break;
        }
        megdnn_throw(ssprintf(
    ...
    static std::unique_ptr<StrategyBase> make_onlypacka_strategy(
        ...
        switch (strategytype) {
            case StrategyType::FLOAT:
                cb1(NCHW, ONLY_PACKA, dt_float32, dt_float32, PostprocessMode::FLOAT,
...
Strategy* StrategyDelegationStorage::get(
        fallback::MatrixMulImpl::AlgoBase* matmul_algo,
        const fallback::ConvBiasImpl::NCBKernSizeParam& param, StrategyType stype) {
    fallback::MatrixMulImpl::AlgoBase::PackMode packmode = matmul_algo->packmode();
    ...
    sparam.is_square = param.filter_meta.spatial[0] == param.filter_meta.spatial[1];
    sparam.is_xcorr = param.filter_meta.should_flip;
    MEGDNN_LOCK_GUARD(m_mtx);
    if (map_strategys.find(sparam) == map_strategys.end()) {
        auto strategy = Factory::make_strategy(matmul_algo, packmode, param, stype);
