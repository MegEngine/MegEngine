...
void get_rectified_size(
        const megdnn::fallback::ConvBiasImpl::NCBKernSizeParam& param, size_t IH,
        size_t IW, size_t OH, size_t OW, size_t FH, size_t FW, size_t PH, size_t PW,
        size_t& IH2, size_t& IW2, size_t& OW2) {
...
WorkspaceBundle MultithreadDirectConvCommon<io_ctype, compute_ctype>::get_bundle(
    ...
    if (param.filter_meta.should_flip) {
        if (m_large_group) {
            ...
            part1 = fm.spatial[0] * fm.spatial[1] * fm.ocpg * fm.icpg * nr_threads *
                    sizeof(io_ctype);
        } else {
...
template <typename io_ctype, typename compute_ctype>
WorkspaceBundle MultithreadDirectConvCommon<io_ctype, compute_ctype>::get_bundle_stride(
        const ConvBiasImpl::NCBKernSizeParam& param, bool m_large_group) {
    ...
    if (need_src_copy(param)) {
        src_size = m_large_group ? IC * IH2 * IW2 * sizeof(io_ctype) * nr_threads
...
void MultithreadDirectConvCommon<io_ctype, compute_ctype>::weight_flip_kern(
    ...
    io_ctype* filter_flip = static_cast<io_ctype*>(bundle.get(1)) +
...
void MultithreadDirectConvCommon<io_ctype, compute_ctype>::copy_padding_kern(
    ...
    size_t batch_id = ncb_index.ndrange_id[1], group_id = ncb_index.ndrange_id[0];
    ...
    } else if (batch_id + 1 == N && channel_id + 1 == IC && group_id + 1 == GROUP) {
        //! copy last plane
...
//! Process one input channel copy padding
template <typename io_ctype, typename compute_ctype>
void MultithreadDirectConvCommon<io_ctype, compute_ctype>::copy_padding_kern_stride(
        const WorkspaceBundle& bundle, const ConvBiasImpl::NCBKernParam& kern_param,
        const ConvBiasImpl::NCBKernIndex& ncb_index, const CpuNDRange& workspace_ids) {
    size_t IH = kern_param.isz[0];
    size_t IW = kern_param.isz[1];
...
void MultithreadDirectConvCommon<io_ctype, compute_ctype>::do_conv_kern(
        const WorkspaceBundle& bundle, const ConvBiasImpl::NCBKernParam& kern_param,
        const ConvBiasImpl::NCBKernIndex& ncb_index, const kern_direct_conv_f32& fun,
    ...
    auto fptr =
            kern_param.filter_meta.should_flip
                    ? static_cast<io_ctype*>(bundle.get(1)) +
                              (workspace_group_id * OC * IC + channel_id * IC) * FH * FW
    ...
    rep(ic, IC) {
        io_ctype* sptr_cur = (ic + 1 == IC ? sptr_last_c : sptr_base + ic * IH2 * IW2);
    ...
    PostProcess<compute_ctype>::run(
            dptr, const_cast<io_ctype*>(bias_ptr), dptr, kern_param.bias_mode,
...
void MultithreadDirectConvCommon<io_ctype, compute_ctype>::do_conv_kern_stride(
        ...
        const kern_direct_conv_f32_stride& fun, const CpuNDRange& workspace_ids) {
    size_t IH = kern_param.isz[0];
    size_t IW = kern_param.isz[1];
    size_t OH = kern_param.osz[0];
    size_t OW = kern_param.osz[1];
    ...
    if (need_dst_copy(kern_param)) {
        dptr_base =
                static_cast<io_ctype*>(bundle.get(1)) + ncb_index.thread_id * OH * OW2;
    } else {
        dptr_base = dptr;
    }
    if (need_dst_copy(kern_param)) {
        ...
        copy_plane_in_bytes(
                dptr, dptr_base, OH, OW * sizeof(io_ctype), OW * sizeof(io_ctype),
                OW2 * sizeof(io_ctype));
    } else {
