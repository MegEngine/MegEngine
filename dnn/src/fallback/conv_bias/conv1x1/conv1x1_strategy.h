...
public:
    virtual void packA(
            ...
            size_t oc_tile_size, const MatrixMulImpl::AlgoBase* matmul_algo,
...
public:
    ...
    void packA(
        ...
        size_t packa_bytes_per_group = oc_tiles_per_group * packa_bytes_per_oc_tile;
        ...
        matmul_kern_param.A_ptr = const_cast<src_ctype*>(
                ncb_param.filter<src_ctype>(group_id) + numbers_offset_of_filter);
        matmul_algo->pack_A(matmul_kern_param, a_panel, 0, oc_end - oc_start);
    }
    ...
    void packB(
        ...
        if (pack_mode == MatrixMulImpl::AlgoBase::PackMode::DEFAULT) {
            ...
            rep(g, GROUP) {
                ...
                size_t bytes_offset_of_b_panel = batch * packb_bytes_per_group * GROUP +
                ...
                matmul_kern_param.B_ptr =
                        const_cast<src_ctype*>(ncb_param.src<src_ctype>(batch, g));
    ...
    void exec(
        ...
        size_t oc_tiles_per_group = div_ceil(OC, oc_tile_size);
        size_t packa_bytes_per_oc_tile = matmul_bundle.get_size(0);
        size_t packa_bytes_per_group = packa_bytes_per_oc_tile * oc_tiles_per_group;
        ...
        size_t bytes_offset_of_b_panel = batch_id * packb_bytes_per_group * GROUP +
        ...
        int8_t* matmul_temp_dst = reinterpret_cast<int8_t*>(whole_bundle.get(2)) +
                                  bytes_offset_of_matmul_dst_this_thread;

        size_t numbers_of_ncb_dst_offset = oc_tile_size * OH * OW * oc_tile_id_in_group;
        ...
        matmul_kern_param.B_ptr =
                const_cast<src_ctype*>(ncb_param.src<src_ctype>(batch_id, group_id));
        ...
        if (pack_mode == MatrixMulImpl::AlgoBase::PackMode::NO_PACK) {
            ...
            auto matmul_kern_naked = matmul_algo->get_kern_naked(matmul_kern_param);
        ...
        PostProcess<op_ctype, op_dtype, postprocess_mode>::run(
                matmul_dst, bias_ptr, conv_bias_dst, param.bias_mode, param.nonlineMode,
                param.bias_type, param.dst_type, 1_z, (oc_end - oc_start) / m_pack_size,
                OH, OW, m_pack_size);
    }
