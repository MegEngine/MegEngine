...
public:
    enum class AlgoType : uint32_t { CAMBRICON_CNNL };
...
class PoolingForwardImpl::AlgoCNNL final : public AlgoBase {
    std::string m_algo_name;
    AlgoAttribute m_algo_attribute;

public:
    AlgoCNNL(std::string name, AlgoAttribute attr)
            : m_algo_name(name), m_algo_attribute(attr) {}

    bool is_available(const SizeArgs& args) const override;
    size_t get_workspace_in_bytes(const SizeArgs& args) const override;
    void init_mode(const ExecArgs& args, cnnlPoolingMode_t& mode) const;
    ...
    MEGDNN_DECL_ALGO_TYPE(CAMBRICON_CNNL)
...
public:
    AlgoPack();
    AlgoCNNL algo_cnnl{"CnnlPoolingForward", AlgoAttribute::REPRODUCIBLE};
...
class PoolingBackwardImpl::AlgoCNNL final : public AlgoBase {
    std::string m_algo_name;
    AlgoAttribute m_algo_attribute;
...
public:
    AlgoPack();
    AlgoCNNL algo_cnnl{"CnnlPoolingBackward", AlgoAttribute::REPRODUCIBLE};
    std::vector<AlgoBase*> all_algos;
