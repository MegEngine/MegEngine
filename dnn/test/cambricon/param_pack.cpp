...
void test_param_pack_concat(
    ...
    if (bench) {
        Timer timer;
        auto* cnrt_handle = static_cast<cambricon::HandleImpl*>(handle);
        cnrtQueueSync(cnrt_handle->queue());
        timer.start();
...
TEST_F(CAMBRICON, PARAM_PACK) {
    ...
    for (auto shapes : shapes_vec) {
        test_param_pack_concat<int32_t>(
                handle_cambricon(), shapes, dtype::Int32(), true);
        test_param_pack_concat<int16_t>(
                handle_cambricon(), shapes, dtype::Int16(), true);
        test_param_pack_concat<float>(
                handle_cambricon(), shapes, dtype::Float32(), true);
    }
}
