...
TEST_F(CAMBRICON, INDEXING_MULTI_AXIS_VEC) {
    run_check<IndexingMultiAxisVec>(handle_cambricon());
    Checker<IndexingMultiAxisVec> checker(handle_cambricon());
    size_t idx_size0;
    OrderedRNG rng_inp;
...
TEST_F(CAMBRICON, INDEXING_MULTI_AXIS_VEC_ND_INDEX) {
    run_check<IndexingMultiAxisVec>(handle_cambricon());
    Checker<IndexingMultiAxisVec> checker(handle_cambricon());
    OrderedRNG rng;
    checker.set_dtype(0, dtype::Float32())
...
TEST_F(CAMBRICON, INDEXING_INCR_MULTI_AXIS_VEC) {
    run_check<IndexingIncrMultiAxisVec>(handle_cambricon());
    Checker<IndexingIncrMultiAxisVec> checker(handle_cambricon());
    OrderedRNG rng;
    checker.set_dtype(0, dtype::Float32())
            .  // data
            set_dtype(1, dtype::Float32())
    ...
    checker.set_dtype(0, dtype::Bool())
            .  // data
...
TEST_F(CAMBRICON, INDEXING_SET_MULTI_AXIS_VEC) {
    Checker<IndexingSetMultiAxisVec> checker(handle_cambricon());
    OrderedRNG rng;
    checker.set_dtype(0, dtype::Float32())
            .  // data
            set_dtype(1, dtype::Float32())
