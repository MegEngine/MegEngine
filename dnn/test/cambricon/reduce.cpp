...
namespace megdnn {
namespace test {
TEST_F(CAMBRICON, REDUCE_FORWARD_DEFAULT) {
    using Param = megdnn::param::Reduce;
    using Mode = Param::Mode;
    auto run_reduce = [&](DType dtype) {
        Checker<ReduceForward> checker(handle_cambricon());
        Param param;
        ...
        for (auto mode :
             {Mode::MIN, Mode::MAX, Mode::SUM, Mode::PRODUCT, Mode::MEAN,
              Mode::SUM_SQR}) {
            param.mode = mode;
            if (mode == Mode::SUM_SQR && dtype == dtype::Int32()) {
                continue;
            }
            for (size_t n : {1, 2}) {
...
TEST_F(CAMBRICON, REDUCE_FORWARD_FLOAT_O32xC32) {
    using Param = megdnn::param::Reduce;
    using Mode = Param::Mode;
    Checker<ReduceForward> checker(handle_cambricon());
    auto run_reduce = [&](DType dtype) {
...
TEST_F(CAMBRICON, REDUCE_FORWARD_FLOAT_O16xC32) {
    using Param = megdnn::param::Reduce;
    using Mode = Param::Mode;
    Checker<ReduceForward> checker(handle_cambricon());
    auto run_reduce = [&](DType dtype) {
