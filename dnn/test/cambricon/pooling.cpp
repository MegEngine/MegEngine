...
TEST_F(CAMBRICON, POOLING_FORWARD) {
    ...
    Checker<Pooling> checker(handle_cambricon());
    ...
    auto run_pooling = [&](size_t N, size_t C, size_t H, size_t W, size_t pad_h,
        ...
        Param param2;
        param2.mode = mode;
        param2.format = Format::NHWC;
        param2.pad_h = pad_h;
        param2.pad_w = pad_w;
        param2.stride_h = stride_h;
        param2.stride_w = stride_w;
        param2.window_h = window_h;
        param2.window_w = window_w;
        checker.set_param(param2).set_dtype(0, dtype0).set_dtype(1, dtype1);
        if (dtype0 == dtype::Float16() || dtype1 == dtype::Float16()) {
            checker.set_epsilon(1e-2);
        }
        checker.execs({{N, H, W, C}, {}});
    };
...
#define RUN_AVG_POOLING(_dt)                                               \
    run_pooling(1, 2048, 7, 7, 0, 0, 1, 1, 7, 7, Mode::AVERAGE, _dt, _dt); \
...
TEST_F(CAMBRICON, POOLING_BACKWARD) {
    using namespace pooling;
    using Param = param::Pooling;
    using Mode = param::Pooling::Mode;
    using Format = param::Pooling::Format;
    ...
    Checker<PoolingBackward> checker(handle_cambricon());
    ...
    auto run_pooling = [&](size_t N, size_t C, size_t H, size_t W, size_t pad_h,
        ...
        checker.execs({{N, H, W, C}, {N, OH, OW, C}, {N, OH, OW, C}, {N, H, W, C}});
    };
