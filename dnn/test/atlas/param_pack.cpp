...
void test_param_pack_concat(
    ...
    TensorND offsets_tensor(offsets.data(), offsets_layout);
    ...
    if (bench) {
        Timer timer;
        auto* atlas_handle = static_cast<atlas::HandleImpl*>(handle);
        aclrtSynchronizeStream(atlas_handle->stream());
        timer.start();
    ...
    for (size_t i = 0; i < pack_size; ++i) {
        ASSERT_EQ(actual_pack[i], expected_pack[i])
                << i << " " << actual_pack[i] << " " << expected_pack[i];
    }
    free(actual_pack);
...
TEST_F(ATLAS, PARAM_PACK) {
    ...
    for (auto shapes : shapes_vec) {
        test_param_pack_concat<int32_t>(handle_atlas(), shapes, dtype::Int32(), true);
        test_param_pack_concat<int16_t>(handle_atlas(), shapes, dtype::Int16(), true);
        test_param_pack_concat<float>(handle_atlas(), shapes, dtype::Float32(), true);
    }
}
