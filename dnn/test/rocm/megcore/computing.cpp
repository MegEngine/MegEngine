...
TEST_F(MegcoreROCM, COMPUTING) {
    for (int id = -1; id < std::min(nr_devices(), 2); ++id) {
        ...
        megcoreCreateDeviceHandle(&devHandle, megcorePlatformROCM, id, 0);
...
TEST_F(MegcoreROCM, STREAM) {
    megcoreDeviceHandle_t devHandle;
    megcoreCreateDeviceHandle(&devHandle, megcorePlatformROCM, 0, 0);
    megcoreActivate(devHandle);
    ...
    megcoreComputingHandle_t compHandle;
    megcoreCreateComputingHandleWithROCMStream(&compHandle, devHandle, 0, stream);
    {
        hipStream_t stream2;
