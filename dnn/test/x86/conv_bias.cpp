...
static void avx2_chanwise_direct_int8x8x32(
        Handle* handle, uint32_t stride, const char* algo) {
    using namespace conv_bias;
...
static void avx2_chanwise_direct_quantizeds32(
        Handle* handle, uint32_t stride, const char* algo) {
    using namespace conv_bias;
    std::vector<TestArg> args;
...
static void avx2_chanwise_direct_quantizeds8x8x8(
        Handle* handle, uint32_t stride, const char* algo) {
    using namespace conv_bias;
    std::vector<TestArg> args;
...
TEST_F(X86_MULTI_THREADS, AVX2_CONV_BIAS_DIRECT_STRIDE1_INT8x8x32) {
    ...
    auto run = [&](size_t oc, size_t ic, size_t w, size_t h, size_t kernel, size_t p,
        ...
        args.emplace_back(
                ...
                TensorShape{2, oc / 2, ic, kernel, kernel}, TensorShape{});
    };
...
TEST_F(X86_MULTI_THREADS, AVX2_CONV_BIAS_DIRECT_STRIDE1_S8S8S8) {
    ...
    auto run = [&](size_t oc, size_t ic, size_t w, size_t h, size_t kernel, size_t p,
        ...
        args.emplace_back(
                param, TensorShape{2, 2 * ic, h, w},
                TensorShape{2, oc / 2, ic, kernel, kernel}, TensorShape{1, oc, 1, 1});
    };
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_DIRECT_STRIDE1_GROUP) {
    ...
    auto run = [&](size_t group, size_t channel, size_t w, size_t h, size_t kernel,
        ...
        args.emplace_back(
                ...
                TensorShape{group, channel / group, channel / group, kernel, kernel},
                TensorShape{
                        2, channel, (h + param.pad_h * 2 - kernel) + 1,
                        (w + param.pad_w * 2 - kernel) + 1});
    };
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_DIRECT_STRIDE2_GROUP) {
    ...
    auto run = [&](size_t group, size_t channel, size_t w, size_t h, size_t kernel,
        ...
        args.emplace_back(
                ...
                TensorShape{group, channel / group, channel / group, kernel, kernel},
                TensorShape{
                        2, channel, (h + param.pad_h * 2 - kernel) / 2 + 1,
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_IM2COLMATMUL_INT8X8X32) {
    ...
    auto run = [&](size_t oc, size_t ic, size_t w, size_t h, size_t kernel, size_t p,
        ...
        args.emplace_back(
                param, TensorShape{1, ic, h, w}, TensorShape{oc, ic, kernel, kernel},
                TensorShape{1, oc, (h + 2 * p - kernel) + 1, (h + 2 * p - kernel) + 1});
    };
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_IM2COLMATMUL_FP32_PACKA) {
    ...
    auto run = [&](size_t oc, size_t ic, size_t w, size_t h, size_t kernel, size_t p,
        ...
        args.emplace_back(
                param, TensorShape{1, 2 * ic, h, w},
                TensorShape{2, oc, ic, kernel, kernel}, TensorShape{1, oc * 2, 1, 1});

        args.emplace_back(
                param, TensorShape{1, 2 * ic, h, w},
                TensorShape{2, oc, ic, kernel, kernel},
                TensorShape{
                        1, 2 * oc, (h + 2 * param.pad_h - kernel) / 1 + 1,
...
void checker_conv_bias_preprocess(
        std::vector<conv_bias::TestArg> args, Handle* handle, RNG* rng, float epsilon,
        DType type0, DType type1, DType type2, DType type3, const char* algo_name) {
    using namespace conv_bias;
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_CONV1X1_S1_FP32_PACKA_PREPROCESS) {
    ...
    checker_conv_bias_preprocess(
            ...
            dtype::Float32{}, dtype::Float32{}, "CONV1x1:X86_F32_MKL_PACKA:24");
}
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_CONV1X1_S1_FP32_BLAS_NOPACK_REPROCESS) {
    ...
    checker_conv_bias_preprocess(
            args, handle(), nullptr, 0.001, dtype::Float32{}, dtype::Float32{},
            dtype::Float32{}, dtype::Float32{}, "CONV1x1:X86_F32_BLAS:24");
}
#endif
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_CONV1X1_S1_INT8X8X32) {
    ...
    if (x86::is_supported(x86::SIMDType::VNNI)) {
        checker_conv_bias(
                ...
                dtype::Int32{}, dtype::Int32{}, "CONV1x1:X86_INT8X8X32_MKLDNN:24");
    }
#endif
#if MEGDNN_X86_WITH_VNNI
    if (x86::is_supported(x86::SIMDType::VNNI)) {
        checker_conv_bias(
                args, handle(), &rng, epsilon, dtype::Int8{}, dtype::Int8{},
                dtype::Int32{}, dtype::Int32{}, "CONV1x1:X86_INT8X8X32_VNNI:24");
    }
#endif
    if (x86::is_supported(x86::SIMDType::AVX2)) {
        checker_conv_bias(
                args, handle(), &rng, epsilon, dtype::Int8{}, dtype::Int8{},
                dtype::Int32{}, dtype::Int32{}, "CONV1x1:X86_INT8X8X32_AVX2_4X16X2:24");
        checker_conv_bias(
                args, handle(), &rng, epsilon, dtype::Int8{}, dtype::Int8{},
                dtype::Int32{}, dtype::Int32{}, "CONV1x1:X86_INT8X8X32_AVX2_2X4X16:24");
        checker_conv_bias(
                args, handle(), &rng, epsilon, dtype::Int8{}, dtype::Int8{},
                dtype::Int16{}, dtype::Int16{}, "CONV1x1:X86_INT8X8X16_AVX2");
    }
    checker_conv_bias(
            args, handle(), &rng, epsilon, dtype::Int8{}, dtype::Int8{}, dtype::Int32{},
            dtype::Int32{}, "CONV1x1:X86_INT8X8X32_SSE_4X8X2:48");
    checker_conv_bias(
            args, handle(), &rng, epsilon, dtype::Int8{}, dtype::Int8{}, dtype::Int16{},
            dtype::Int16{}, "CONV1x1:X86_INT8X8X16_SSE");
}
...
static void x86_benchmark_fp32_mkldnn(Handle* handle) {
    ...
    auto run = [&](size_t N, size_t IC, size_t OC, size_t H, size_t W, size_t FS,
        ...
        auto im2col_used =
                benchmarker_im2col.set_param(param).exec({src, filter, bias, z, dst}) /
                RUNS;

        src = IC < 8 ? TensorShape{N, IC, H, W} : TensorShape{N, IC / 8, H, W, 8};
        ...
        auto mkldnn_used =
                benchmarker_mkldnn.set_param(param).exec({src, filter, bias, z, dst}) /
                RUNS;
        float computations =
                (IC / GROUP * FS * FS + 1) * dst.total_nr_elems() * 2 * 1e-6;
        std::cout << "run " << src.to_string() << " " << filter.to_string() << " "
                  << bias.to_string() << " " << dst.to_string() << std::endl;
...
std::vector<conv_bias::TestArg> get_winograd_mk_nchw88_args() {
    ...
    for (size_t i : {9, 63}) {
        ...
        cur_param.pad_h = cur_param.pad_w = 1;
        args.emplace_back(
                cur_param, TensorShape{2, 1, 9, 9, 8}, TensorShape{128, 1, 3, 3, 8, 8},
...
TEST_F(X86_MULTI_THREADS, CONV_BIAS_WINOGRAD_WEIGHT_PREPROCESS) {
    ...
    run(args, dtype::Float32(), dtype::Float32(), dtype::Float32(), dtype::Float32(),
        1e-3f);
}
...
static void x86_correctness_fp32_mkldnn_run(
        ...
        ConvBiasForward::BiasMode bias_mode, param::ConvBias::NonlineMode noline_mode,
        size_t n, size_t stride, size_t kernel, size_t oc, size_t ic, size_t h,
        size_t w, size_t group) {
    ...
    if (bias_mode == megdnn::BiasMode::BROADCAST_CHANNEL_BIAS) {
        ...
        ConvBiasOp->deduce_layout(
                {src_tensor_shape, dtype::Float32()},
                {weight_tensor_shape, dtype::Float32()}, {}, {}, dst_layout);
    ...
    checker.set_dtype(0, dtype::Float32())
            ...
            .set_epsilon(1e-3)
            .set_param(param)
            .execs({src_tensor_shape, weight_tensor_shape, bias_tensor_shape, {}, {}});
}
...
static void x86_correctness_fp32_mkldnn(Handle* handle) {
    ...
    checker.set_before_exec_callback(
            conv_bias::ConvBiasAlgoChecker<ConvBiasForward>("MKLDNN_CONV_FP32"));

    for (auto bias_mode :
         {megdnn::BiasMode::NO_BIAS, megdnn::BiasMode::BROADCAST_CHANNEL_BIAS,
          megdnn::BiasMode::BIAS})
        for (auto noline_mode :
                                    ...
                                    for (size_t w : {22, 33}) {
                                        for (size_t group = 1;
                                            ...
                                            x86_correctness_fp32_mkldnn_run(
                                                    checker, rng, handle, bias_mode,
                                                    noline_mode, n, stride, kernel, oc,
                                                    ic, h, w, group);
                                        }
                                    }
}
...
TEST_F(X86, CONV_BIAS_MKL_DNN_MATMUL_INT8) {
    ...
    Checker<ConvBias> checker(handle());
    checker.set_before_exec_callback(
            conv_bias::ConvBiasAlgoChecker<ConvBiasForward>("MKLDNN_MATMUL_INT8"));
...
void benchmark_impl_comp(
    ...
    }
    {
        ...
        benchmarker.set_times(RUNS)
                ...
                .set_param(param)
                .set_before_exec_callback(
                        conv_bias::ConvBiasAlgoChecker<ConvBias>(algo_name1.c_str()));
        for (auto shape : shapes_and_computation) {
            single_thread_times.push_back(benchmarker.exec(shape.first) / RUNS);
        }
    }
...
static void benchmark_convbias_chanwise_avx2_int8(uint32_t stride, const char* algo) {
    ...
    auto bench_case = [&](size_t N, size_t IC, size_t H, size_t W, size_t FS) {
        ...
        SmallVector<TensorShape> shapes{{N, IC, H, W}, {IC, 1, 1, FS, FS}, {}, {}, {}};
        TensorShape dst{
                N, IC, (H + 2 * param.pad_h - FS) + 1, (W + 2 * param.pad_w - FS) + 1};
...
TEST_F(X86_BENCHMARK_MULTI_THREADS, BENCHMARK_CONVBIAS_DIRECT_AVX2_INT8) {
    ...
    auto bench_case = [&](size_t N, size_t IC, size_t OC, size_t H, size_t W,
        ...
        SmallVector<TensorShape> shapes{{N, IC, H, W}, {OC, IC, FS, FS}, {}, {}, {}};
        TensorShape dst{
                N, OC, (H + 2 * param.pad_h - FS) + 1, (W + 2 * param.pad_w - FS) + 1};
...
TEST_F(X86_BENCHMARK_MULTI_THREADS, BENCHMARK_CONVBIAS_8816) {
    ...
    std::vector<DType> data_type = {
            dtype::Int8(), dtype::Int8(), dtype::Int16(), dtype::Int16()};
    ...
    auto bench_case = [&](size_t N, size_t IC, size_t OC, size_t H, size_t W,
        ...
        SmallVector<TensorShape> shapes{{N, IC, H, W}, {OC, IC, FS, FS}, {}, {}, {}};
        TensorShape dst{
                N, OC, (H + 2 * param.pad_h - FS) / param.stride_h + 1,
...
TEST_F(X86_BENCHMARK_MULTI_THREADS, BENCHMARK_CONVBIAS_DIRECT_AVX2_INT8_STRIDE2) {
    constexpr size_t RUNS = 50;
    param::ConvBias param;
    param.stride_h = 2;
    param.stride_w = 2;
