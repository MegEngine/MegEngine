...
class member_defs:
    ...
    Dtype = collections.namedtuple(
        "Dtype", ["cname", "pycvt", "pyfmt", "cppjson", "cname_attr"]
    )
...
class ParamDef:
    ...
    def __init__(self, name, doc="", *, version=0, is_legacy=False):
        ...
        if self.tag < 1024:
            self.tag += 1024
        assert (
            self.tag not in self.__all_tags
    ...
    def add_bit_combination_enum(
        self, name, *members, default=tuple(), name_field=None, member_alias=[]
    ):
        self.members.append(
            member_defs.Enum(
    ...
    def add_enum_alias(
        ...
        self.members.append(
            member_defs.EnumAlias(name, name_field, src_class, src_name, default)
        )
        return self
...
'''
    ...
    def _on_param_end(self, p):
        ...
        self._write(
            "__slots__ = [%s]", ", ".join(map('"{.name}"'.format, self._cur_fields))
        )
        ...
        self._write(
            ...
            ", ".join(
                ["self"]
                + list("{}={}".format(i.name, i.default) for i in self._cur_fields)
            ),
            indent=1,
        )
        # gen __init__ doc
...
class CPPWriter(IndentWriterBase):
    ...
    def _add_ctor_args(self, typename, default, varname):
        self._ctor_args.append(
            ("{} {}_={}".format(typename, varname, default), varname)
        )
    ...
    def _on_member_enum(self, e):
        ...
        for mem, alias in e.member_alias:
            if e.combined:
                ...
                self._write(
                    "%s = %s,",
                    str(alias).split(" ")[0].split("=")[0],
                    str(mem).split(" ")[0].split("=")[0],
                )
        self._write("};", indent=-1)
        ...
        if e.combined:
            default = "static_cast<{}>({})".format(
                e.name, e.compose_combined_enum(e.default)
            )
        else:
    ...
    def _on_member_enum_alias(self, e):
        ...
        if s.combined:
            default = "static_cast<{}>({})".format(
                e.name, s.compose_combined_enum(e.default)
            )
        else:
...
class CPPParamJsonFuncWriter(IndentWriterBase):
    ...
    def _write_json_item(self, json_cls, field):
        ...
        self._items.append(
            '{"%s", json::%s::make(static_cast<%s>(p.%s))},'
    ...
    def __call__(self, fout, defs):
        ...
        self._write("// %s", self._get_header())
        self._write(
            "// this file can only be included in "
    ...
    def _on_member_enum(self, e):
        self._write(
            "auto %s2str = [](const %s::%s arg) -> std::string {",
            e.name,
            self._param_name,
            e.name,
            indent=1,
...
def main():
    ...
    parser.add_argument(
        "--enumv",
        action="store_true",
        help="generate c++03 compatible code which only " "contains enum values",
    )
    parser.add_argument(
        "-t", "--type", choices=["c++", "py"], default="c++", help="output type"
    )
    parser.add_argument(
        "--write-enum-items",
    ...
    parser.add_argument(
        "--write-cppjson",
        help="generate megbrain json serialization implemention" "cpp file",
    )
    parser.add_argument("input")
    parser.add_argument("output")
    parser.add_argument(
        "--imperative", action="store_true", help="generate files for imperatvie "
    )
